// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TermService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class TermService : ServiceBase, ITermService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Term"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.TermDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.TermDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.TermDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.TermDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.TermDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.TermDao.Update(transaction, partition, term, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var term = thing as Term;
            var createSuccesful = this.TermDao.Write(transaction, partition, term, container);
            return createSuccesful && this.CreateContainment(transaction, partition, term);
        }
        ...
        {
            var term = thing as Term;
            var createSuccesful = this.TermDao.Upsert(transaction, partition, term, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, term);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Term"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var termColl = new List<Thing>(this.TermDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(termColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Term"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, termColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, termColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, termColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, termColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, Term term)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(term.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, term));
            }

            foreach (var attachment in this.ResolveFromRequestCache(term.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, term));
            }

            foreach (var definition in this.ResolveFromRequestCache(term.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, term));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(term.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, term));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, Term term)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(term.Alias))
            {
                results.Add(this.AliasService.UpsertConcept(transaction, partition, alias, term));
            }

            foreach (var attachment in this.ResolveFromRequestCache(term.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, term));
            }

            foreach (var definition in this.ResolveFromRequestCache(term.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, term));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(term.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, term));
            }
