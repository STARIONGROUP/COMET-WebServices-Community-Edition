// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GoalService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class GoalService : ServiceBase, IGoalService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Goal"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.GoalDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.GoalDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.GoalDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.GoalDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.GoalDao.Update(transaction, partition, goal, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var goal = thing as Goal;
            var createSuccesful = this.GoalDao.Write(transaction, partition, goal, container);
            return createSuccesful && this.CreateContainment(transaction, partition, goal);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Goal"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var goalColl = new List<Thing>(this.GoalDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(goalColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Goal"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, goalColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, goalColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, goalColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, goalColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, Goal goal)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(goal.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, goal));
            }

            foreach (var attachment in this.ResolveFromRequestCache(goal.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, goal));
            }

            foreach (var definition in this.ResolveFromRequestCache(goal.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, goal));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(goal.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, goal));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, Goal goal)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(goal.Alias))
            {
                results.Add(this.AliasService.UpsertConcept(transaction, partition, alias, goal));
            }

            foreach (var attachment in this.ResolveFromRequestCache(goal.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, goal));
            }

            foreach (var definition in this.ResolveFromRequestCache(goal.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, goal));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(goal.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, goal));
            }
