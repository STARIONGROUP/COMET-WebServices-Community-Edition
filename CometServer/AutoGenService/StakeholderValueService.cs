...
namespace CometServer.Services
{
    ...
    public sealed partial class StakeholderValueService : ServiceBase, IStakeholderValueService
    {
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.StakeholderValueDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        {
            var stakeholderValue = thing as StakeholderValue;
            var createSuccesful = this.StakeholderValueDao.Upsert(transaction, partition, stakeholderValue, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, stakeholderValue);
        }
        ...
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var stakeholderValueColl = new List<Thing>(this.StakeholderValueDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, stakeholderValueColl.SelectMany(x => x.Attachment), containerSecurityContext));
        ...
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, StakeholderValue stakeholderValue)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(stakeholderValue.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, stakeholderValue));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, StakeholderValue stakeholderValue)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(stakeholderValue.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, stakeholderValue));
            }
