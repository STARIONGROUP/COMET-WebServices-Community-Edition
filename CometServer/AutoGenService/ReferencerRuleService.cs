// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ReferencerRuleService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class ReferencerRuleService : ServiceBase, IReferencerRuleService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ReferencerRule"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.ReferencerRuleDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ReferencerRuleDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ReferencerRuleDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ReferencerRuleDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ReferencerRuleDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.ReferencerRuleDao.Update(transaction, partition, referencerRule, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var referencerRule = thing as ReferencerRule;
            var createSuccesful = this.ReferencerRuleDao.Write(transaction, partition, referencerRule, container);
            return createSuccesful && this.CreateContainment(transaction, partition, referencerRule);
        }
        ...
        {
            var referencerRule = thing as ReferencerRule;
            var createSuccesful = this.ReferencerRuleDao.Upsert(transaction, partition, referencerRule, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, referencerRule);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ReferencerRule"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var referencerRuleColl = new List<Thing>(this.ReferencerRuleDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(referencerRuleColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ReferencerRule"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, referencerRuleColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, referencerRuleColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, referencerRuleColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, referencerRuleColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, ReferencerRule referencerRule)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(referencerRule.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, referencerRule));
            }

            foreach (var attachment in this.ResolveFromRequestCache(referencerRule.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, referencerRule));
            }

            foreach (var definition in this.ResolveFromRequestCache(referencerRule.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, referencerRule));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(referencerRule.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, referencerRule));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, ReferencerRule referencerRule)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(referencerRule.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, referencerRule));
            }
