// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DateParameterTypeService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class DateParameterTypeService : ServiceBase, IDateParameterTypeService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="DateParameterType"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.DateParameterTypeDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.DateParameterTypeDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.DateParameterTypeDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.DateParameterTypeDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.DateParameterTypeDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.DateParameterTypeDao.Update(transaction, partition, dateParameterType, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var dateParameterType = thing as DateParameterType;
            var createSuccesful = this.DateParameterTypeDao.Write(transaction, partition, dateParameterType, container);
            return createSuccesful && this.CreateContainment(transaction, partition, dateParameterType);
        }
        ...
        {
            var dateParameterType = thing as DateParameterType;
            var createSuccesful = this.DateParameterTypeDao.Upsert(transaction, partition, dateParameterType, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, dateParameterType);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="DateParameterType"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var dateParameterTypeColl = new List<Thing>(this.DateParameterTypeDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(dateParameterTypeColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="DateParameterType"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, dateParameterTypeColl.SelectMany(x => x.Attachment), containerSecurityContext));
            ...
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, dateParameterTypeColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, DateParameterType dateParameterType)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(dateParameterType.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, dateParameterType));
            }

            foreach (var attachment in this.ResolveFromRequestCache(dateParameterType.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, dateParameterType));
            }

            foreach (var definition in this.ResolveFromRequestCache(dateParameterType.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, dateParameterType));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(dateParameterType.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, dateParameterType));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, DateParameterType dateParameterType)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(dateParameterType.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, dateParameterType));
            }
