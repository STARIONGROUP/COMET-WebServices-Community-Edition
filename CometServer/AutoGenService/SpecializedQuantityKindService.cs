// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SpecializedQuantityKindService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class SpecializedQuantityKindService : ServiceBase, ISpecializedQuantityKindService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="SpecializedQuantityKind"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.SpecializedQuantityKindDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.SpecializedQuantityKindDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.SpecializedQuantityKindDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.SpecializedQuantityKindDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.SpecializedQuantityKindDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.SpecializedQuantityKindDao.Update(transaction, partition, specializedQuantityKind, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var specializedQuantityKind = thing as SpecializedQuantityKind;
            var createSuccesful = this.SpecializedQuantityKindDao.Write(transaction, partition, specializedQuantityKind, container);
            return createSuccesful && this.CreateContainment(transaction, partition, specializedQuantityKind);
        }
        ...
        {
            var specializedQuantityKind = thing as SpecializedQuantityKind;
            var createSuccesful = this.SpecializedQuantityKindDao.Upsert(transaction, partition, specializedQuantityKind, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, specializedQuantityKind);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="SpecializedQuantityKind"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var specializedQuantityKindColl = new List<Thing>(this.SpecializedQuantityKindDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(specializedQuantityKindColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="SpecializedQuantityKind"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, specializedQuantityKindColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, specializedQuantityKindColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, specializedQuantityKindColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, specializedQuantityKindColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, SpecializedQuantityKind specializedQuantityKind)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(specializedQuantityKind.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, specializedQuantityKind));
            }

            foreach (var attachment in this.ResolveFromRequestCache(specializedQuantityKind.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, specializedQuantityKind));
            }

            foreach (var definition in this.ResolveFromRequestCache(specializedQuantityKind.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, specializedQuantityKind));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(specializedQuantityKind.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, specializedQuantityKind));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, SpecializedQuantityKind specializedQuantityKind)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(specializedQuantityKind.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, specializedQuantityKind));
            }
