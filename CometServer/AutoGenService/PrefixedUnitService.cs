// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PrefixedUnitService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class PrefixedUnitService : ServiceBase, IPrefixedUnitService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="PrefixedUnit"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.PrefixedUnitDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.PrefixedUnitDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.PrefixedUnitDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.PrefixedUnitDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.PrefixedUnitDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.PrefixedUnitDao.Update(transaction, partition, prefixedUnit, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var prefixedUnit = thing as PrefixedUnit;
            var createSuccesful = this.PrefixedUnitDao.Write(transaction, partition, prefixedUnit, container);
            return createSuccesful && this.CreateContainment(transaction, partition, prefixedUnit);
        }
        ...
        {
            var prefixedUnit = thing as PrefixedUnit;
            var createSuccesful = this.PrefixedUnitDao.Upsert(transaction, partition, prefixedUnit, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, prefixedUnit);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="PrefixedUnit"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var prefixedUnitColl = new List<Thing>(this.PrefixedUnitDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(prefixedUnitColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="PrefixedUnit"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, prefixedUnitColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, prefixedUnitColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, prefixedUnitColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, prefixedUnitColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, PrefixedUnit prefixedUnit)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(prefixedUnit.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, prefixedUnit));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, PrefixedUnit prefixedUnit)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(prefixedUnit.Alias))
            {
                results.Add(this.AliasService.UpsertConcept(transaction, partition, alias, prefixedUnit));
            }

            foreach (var attachment in this.ResolveFromRequestCache(prefixedUnit.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, prefixedUnit));
            }

            foreach (var definition in this.ResolveFromRequestCache(prefixedUnit.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, prefixedUnit));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(prefixedUnit.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, prefixedUnit));
            }
