// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParameterizedCategoryRuleService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class ParameterizedCategoryRuleService : ServiceBase, IParameterizedCategoryRuleService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ParameterizedCategoryRule"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.ParameterizedCategoryRuleDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ParameterizedCategoryRuleDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ParameterizedCategoryRuleDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ParameterizedCategoryRuleDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ParameterizedCategoryRuleDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.ParameterizedCategoryRuleDao.Update(transaction, partition, parameterizedCategoryRule, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var parameterizedCategoryRule = thing as ParameterizedCategoryRule;
            var createSuccesful = this.ParameterizedCategoryRuleDao.Write(transaction, partition, parameterizedCategoryRule, container);
            return createSuccesful && this.CreateContainment(transaction, partition, parameterizedCategoryRule);
        }
        ...
        {
            var parameterizedCategoryRule = thing as ParameterizedCategoryRule;
            var createSuccesful = this.ParameterizedCategoryRuleDao.Upsert(transaction, partition, parameterizedCategoryRule, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, parameterizedCategoryRule);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ParameterizedCategoryRule"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var parameterizedCategoryRuleColl = new List<Thing>(this.ParameterizedCategoryRuleDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(parameterizedCategoryRuleColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ParameterizedCategoryRule"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, parameterizedCategoryRuleColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, parameterizedCategoryRuleColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, parameterizedCategoryRuleColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, parameterizedCategoryRuleColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, ParameterizedCategoryRule parameterizedCategoryRule)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(parameterizedCategoryRule.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, parameterizedCategoryRule));
            }

            foreach (var attachment in this.ResolveFromRequestCache(parameterizedCategoryRule.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, parameterizedCategoryRule));
            }

            foreach (var definition in this.ResolveFromRequestCache(parameterizedCategoryRule.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, parameterizedCategoryRule));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(parameterizedCategoryRule.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, parameterizedCategoryRule));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, ParameterizedCategoryRule parameterizedCategoryRule)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(parameterizedCategoryRule.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, parameterizedCategoryRule));
            }
