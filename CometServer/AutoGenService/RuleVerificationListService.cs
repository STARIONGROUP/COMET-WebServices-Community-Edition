...
namespace CometServer.Services
{
    ...
    public sealed partial class RuleVerificationListService : ServiceBase, IRuleVerificationListService
    {
        ...
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var ruleVerificationListColl = new List<Thing>(this.RuleVerificationListDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, ruleVerificationListColl.SelectMany(x => x.Attachment), containerSecurityContext));
        ...
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, RuleVerificationList ruleVerificationList)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(ruleVerificationList.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, ruleVerificationList));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, RuleVerificationList ruleVerificationList)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(ruleVerificationList.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, ruleVerificationList));
            }
