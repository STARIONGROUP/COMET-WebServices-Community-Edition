...
namespace CometServer.Services
{
    ...
    public sealed partial class ChangeProposalService : ServiceBase, IChangeProposalService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ChangeProposal"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.ChangeProposalDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ChangeProposalDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ChangeProposalDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ChangeProposalDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ChangeProposalDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.ChangeProposalDao.Update(transaction, partition, changeProposal, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var changeProposal = thing as ChangeProposal;
            var createSuccesful = this.ChangeProposalDao.Write(transaction, partition, changeProposal, container);
            return createSuccesful && this.CreateContainment(transaction, partition, changeProposal);
        }
        ...
        {
            var changeProposal = thing as ChangeProposal;
            var createSuccesful = this.ChangeProposalDao.Upsert(transaction, partition, changeProposal, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, changeProposal);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ChangeProposal"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var changeProposalColl = new List<Thing>(this.ChangeProposalDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(changeProposalColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="ChangeProposal"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.RelatedThingService.GetDeep(transaction, partition, changeProposalColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, ChangeProposal changeProposal)
        {
            var results = new List<bool>();

            foreach (var approvedBy in this.ResolveFromRequestCache(changeProposal.ApprovedBy))
            {
                results.Add(this.ApprovedByService.CreateConcept(transaction, partition, approvedBy, changeProposal));
            }

            foreach (var discussion in this.ResolveFromRequestCache(changeProposal.Discussion))
            {
                results.Add(this.DiscussionService.CreateConcept(transaction, partition, discussion, changeProposal));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(changeProposal.RelatedThing))
            {
                results.Add(this.RelatedThingService.CreateConcept(transaction, partition, relatedThing, changeProposal));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, ChangeProposal changeProposal)
        {
            var results = new List<bool>();

            foreach (var approvedBy in this.ResolveFromRequestCache(changeProposal.ApprovedBy))
            {
                results.Add(this.ApprovedByService.UpsertConcept(transaction, partition, approvedBy, changeProposal));
            }

            foreach (var discussion in this.ResolveFromRequestCache(changeProposal.Discussion))
            {
                results.Add(this.DiscussionService.UpsertConcept(transaction, partition, discussion, changeProposal));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(changeProposal.RelatedThing))
            {
                results.Add(this.RelatedThingService.UpsertConcept(transaction, partition, relatedThing, changeProposal));
            }
