// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PersonRoleService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class PersonRoleService : ServiceBase, IPersonRoleService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="PersonRole"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.PersonRoleDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.PersonRoleDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.PersonRoleDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.PersonRoleDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.PersonRoleDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.PersonRoleDao.Update(transaction, partition, personRole, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var personRole = thing as PersonRole;
            var createSuccesful = this.PersonRoleDao.Write(transaction, partition, personRole, container);
            return createSuccesful && this.CreateContainment(transaction, partition, personRole);
        }
        ...
        {
            var personRole = thing as PersonRole;
            var createSuccesful = this.PersonRoleDao.Upsert(transaction, partition, personRole, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, personRole);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="PersonRole"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var personRoleColl = new List<Thing>(this.PersonRoleDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(personRoleColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="PersonRole"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, personRoleColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, personRoleColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, personRoleColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, personRoleColl.SelectMany(x => x.HyperLink), containerSecurityContext));
            results.AddRange(this.PersonPermissionService.GetDeep(transaction, partition, personRoleColl.SelectMany(x => x.PersonPermission), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, PersonRole personRole)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(personRole.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, personRole));
            }

            foreach (var attachment in this.ResolveFromRequestCache(personRole.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, personRole));
            }

            foreach (var definition in this.ResolveFromRequestCache(personRole.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, personRole));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(personRole.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, personRole));
            }

            foreach (var personPermission in this.ResolveFromRequestCache(personRole.PersonPermission))
            {
                results.Add(this.PersonPermissionService.CreateConcept(transaction, partition, personPermission, personRole));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, PersonRole personRole)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(personRole.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, personRole));
            }
