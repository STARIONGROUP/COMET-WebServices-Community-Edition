// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DecompositionRuleService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class DecompositionRuleService : ServiceBase, IDecompositionRuleService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="DecompositionRule"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.DecompositionRuleDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.DecompositionRuleDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.DecompositionRuleDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.DecompositionRuleDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.DecompositionRuleDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.DecompositionRuleDao.Update(transaction, partition, decompositionRule, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var decompositionRule = thing as DecompositionRule;
            var createSuccesful = this.DecompositionRuleDao.Write(transaction, partition, decompositionRule, container);
            return createSuccesful && this.CreateContainment(transaction, partition, decompositionRule);
        }
        ...
        {
            var decompositionRule = thing as DecompositionRule;
            var createSuccesful = this.DecompositionRuleDao.Upsert(transaction, partition, decompositionRule, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, decompositionRule);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="DecompositionRule"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var decompositionRuleColl = new List<Thing>(this.DecompositionRuleDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(decompositionRuleColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="DecompositionRule"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, decompositionRuleColl.SelectMany(x => x.Attachment), containerSecurityContext));
            ...
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, decompositionRuleColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, DecompositionRule decompositionRule)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(decompositionRule.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, decompositionRule));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, DecompositionRule decompositionRule)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(decompositionRule.Alias))
            {
                results.Add(this.AliasService.UpsertConcept(transaction, partition, alias, decompositionRule));
            }

            foreach (var attachment in this.ResolveFromRequestCache(decompositionRule.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, decompositionRule));
            }

            foreach (var definition in this.ResolveFromRequestCache(decompositionRule.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, decompositionRule));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(decompositionRule.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, decompositionRule));
            }
