// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ReviewItemDiscrepancyService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class ReviewItemDiscrepancyService : ServiceBase, IReviewItemDiscrepancyService
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="ReviewItemDiscrepancy"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        {
            return await this.ReviewItemDiscrepancyDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.ReviewItemDiscrepancyDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.ReviewItemDiscrepancyDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.ReviewItemDiscrepancyDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.ReviewItemDiscrepancyDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.ReviewItemDiscrepancyDao.UpdateAsync(transaction, partition, reviewItemDiscrepancy, container);
        }
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var reviewItemDiscrepancy = thing as ReviewItemDiscrepancy;
            var createSuccesful = await this.ReviewItemDiscrepancyDao.WriteAsync(transaction, partition, reviewItemDiscrepancy, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, reviewItemDiscrepancy);
        }
        ...
        {
            var reviewItemDiscrepancy = thing as ReviewItemDiscrepancy;
            var createSuccesful = await this.ReviewItemDiscrepancyDao.UpsertAsync(transaction, partition, reviewItemDiscrepancy, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, reviewItemDiscrepancy);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="ReviewItemDiscrepancy"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var reviewItemDiscrepancyColl = new List<Thing>(await this.ReviewItemDiscrepancyDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(reviewItemDiscrepancyColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="ReviewItemDiscrepancy"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.ApprovedByService.GetDeepAsync(transaction, partition, reviewItemDiscrepancyColl.SelectMany(x => x.ApprovedBy), containerSecurityContext));
            results.AddRange(await this.DiscussionService.GetDeepAsync(transaction, partition, reviewItemDiscrepancyColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(await this.RelatedThingService.GetDeepAsync(transaction, partition, reviewItemDiscrepancyColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
            results.AddRange(await this.SolutionService.GetDeepAsync(transaction, partition, reviewItemDiscrepancyColl.SelectMany(x => x.Solution), containerSecurityContext));
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, ReviewItemDiscrepancy reviewItemDiscrepancy)
        {
            var results = new List<bool>();

            foreach (var approvedBy in this.ResolveFromRequestCache(reviewItemDiscrepancy.ApprovedBy))
            {
                results.Add(await this.ApprovedByService.CreateConceptAsync(transaction, partition, approvedBy, reviewItemDiscrepancy));
            }

            foreach (var discussion in this.ResolveFromRequestCache(reviewItemDiscrepancy.Discussion))
            {
                results.Add(await this.DiscussionService.CreateConceptAsync(transaction, partition, discussion, reviewItemDiscrepancy));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(reviewItemDiscrepancy.RelatedThing))
            {
                results.Add(await this.RelatedThingService.CreateConceptAsync(transaction, partition, relatedThing, reviewItemDiscrepancy));
            }

            foreach (var solution in this.ResolveFromRequestCache(reviewItemDiscrepancy.Solution))
            {
                results.Add(await this.SolutionService.CreateConceptAsync(transaction, partition, solution, reviewItemDiscrepancy));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, ReviewItemDiscrepancy reviewItemDiscrepancy)
        {
            var results = new List<bool>();

            foreach (var approvedBy in this.ResolveFromRequestCache(reviewItemDiscrepancy.ApprovedBy))
            {
                results.Add(await this.ApprovedByService.UpsertConceptAsync(transaction, partition, approvedBy, reviewItemDiscrepancy));
            }

            foreach (var discussion in this.ResolveFromRequestCache(reviewItemDiscrepancy.Discussion))
            {
                results.Add(await this.DiscussionService.UpsertConceptAsync(transaction, partition, discussion, reviewItemDiscrepancy));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(reviewItemDiscrepancy.RelatedThing))
            {
                results.Add(await this.RelatedThingService.UpsertConceptAsync(transaction, partition, relatedThing, reviewItemDiscrepancy));
            }

            foreach (var solution in this.ResolveFromRequestCache(reviewItemDiscrepancy.Solution))
            {
                results.Add(await this.SolutionService.UpsertConceptAsync(transaction, partition, solution, reviewItemDiscrepancy));
            }
