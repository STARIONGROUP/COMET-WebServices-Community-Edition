...
namespace CometServer.Services
{
    ...
    public sealed partial class ParameterService : ServiceBase, IParameterService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Parameter"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.ParameterDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ParameterDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.ParameterDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ParameterDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.ParameterDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.ParameterDao.Update(transaction, partition, parameter, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var parameter = thing as Parameter;
            var createSuccesful = this.ParameterDao.Write(transaction, partition, parameter, container);
            return createSuccesful && this.CreateContainment(transaction, partition, parameter);
        }
        ...
        {
            var parameter = thing as Parameter;
            var createSuccesful = this.ParameterDao.Upsert(transaction, partition, parameter, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, parameter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Parameter"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var parameterColl = new List<Thing>(this.ParameterDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(parameterColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Parameter"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.ParameterSubscriptionService.GetDeep(transaction, partition, parameterColl.SelectMany(x => x.ParameterSubscription), containerSecurityContext));
            results.AddRange(this.ValueSetService.GetDeep(transaction, partition, parameterColl.SelectMany(x => x.ValueSet), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, Parameter parameter)
        {
            var results = new List<bool>();

            foreach (var parameterSubscription in this.ResolveFromRequestCache(parameter.ParameterSubscription))
            {
                results.Add(this.ParameterSubscriptionService.CreateConcept(transaction, partition, parameterSubscription, parameter));
            }

            foreach (var valueSet in this.ResolveFromRequestCache(parameter.ValueSet))
            {
                results.Add(this.ValueSetService.CreateConcept(transaction, partition, valueSet, parameter));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, Parameter parameter)
        {
            var results = new List<bool>();

            foreach (var parameterSubscription in this.ResolveFromRequestCache(parameter.ParameterSubscription))
            {
                results.Add(this.ParameterSubscriptionService.UpsertConcept(transaction, partition, parameterSubscription, parameter));
            }

            foreach (var valueSet in this.ResolveFromRequestCache(parameter.ValueSet))
            {
                results.Add(this.ValueSetService.UpsertConcept(transaction, partition, valueSet, parameter));
            }
