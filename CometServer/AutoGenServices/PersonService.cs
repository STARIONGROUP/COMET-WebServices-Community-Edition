...
namespace CometServer.Services
{
    ...
    /// <summary>
    /// The <see cref="Person"/> Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class PersonService : ServiceBase, IPersonService
    {
        /// <summary>
        ...
        /// <summary>
        /// Get the requested <see cref="Person"/>s from the ORM layer.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="Person"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="Person"/> id that will be the source for each link table record.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.PersonDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="Person"/> id that is the source of the link table records.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.PersonDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="Person"/> id that is the source for the reordered link table record.
        /// </param>
        ...
        {
            return await this.PersonDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.PersonDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.PersonDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="Person"/> <see cref="Thing"/> to update.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="Person"/> to be updated.
        /// </param>
        /// <returns>
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.PersonDao.UpdateAsync(transaction, partition, person, container);
        }

        /// <summary>
        /// Persist the supplied <see cref="Person"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var person = thing as Person;
            var createSuccesful = await this.PersonDao.WriteAsync(transaction, partition, person, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, person);
        }
        ...
        {
            var person = thing as Person;
            var createSuccesful = await this.PersonDao.UpsertAsync(transaction, partition, person, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, person);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Person"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var authorizedContext = this.AuthorizeReadRequest("Person", containerSecurityContext, partition);
            ...
            var personColl = new List<Thing>(await this.PersonDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(personColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Person"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var personColl = results.Where(i => i.GetType() == typeof(Person)).Cast<Person>().ToList();

            results.AddRange(await this.EmailAddressService.GetDeepAsync(transaction, partition, personColl.SelectMany(x => x.EmailAddress), containerSecurityContext));
            results.AddRange(await this.TelephoneNumberService.GetDeepAsync(transaction, partition, personColl.SelectMany(x => x.TelephoneNumber), containerSecurityContext));
            results.AddRange(await this.UserPreferenceService.GetDeepAsync(transaction, partition, personColl.SelectMany(x => x.UserPreference), containerSecurityContext));
        ...
        /// <summary>
        /// Persist the <see cref="Person"/> containment tree to the ORM layer.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, Person person)
        {
            var results = new List<bool>();

            foreach (var emailAddress in this.ResolveFromRequestCache(person.EmailAddress))
            {
                results.Add(await this.EmailAddressService.CreateConceptAsync(transaction, partition, emailAddress, person));
            }

            foreach (var telephoneNumber in this.ResolveFromRequestCache(person.TelephoneNumber))
            {
                results.Add(await this.TelephoneNumberService.CreateConceptAsync(transaction, partition, telephoneNumber, person));
            }

            foreach (var userPreference in this.ResolveFromRequestCache(person.UserPreference))
            {
                results.Add(await this.UserPreferenceService.CreateConceptAsync(transaction, partition, userPreference, person));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, Person person)
        {
            var results = new List<bool>();

            foreach (var emailAddress in this.ResolveFromRequestCache(person.EmailAddress))
            {
                results.Add(await this.EmailAddressService.UpsertConceptAsync(transaction, partition, emailAddress, person));
            }

            foreach (var telephoneNumber in this.ResolveFromRequestCache(person.TelephoneNumber))
            {
                results.Add(await this.TelephoneNumberService.UpsertConceptAsync(transaction, partition, telephoneNumber, person));
            }

            foreach (var userPreference in this.ResolveFromRequestCache(person.UserPreference))
            {
                results.Add(await this.UserPreferenceService.UpsertConceptAsync(transaction, partition, userPreference, person));
            }
