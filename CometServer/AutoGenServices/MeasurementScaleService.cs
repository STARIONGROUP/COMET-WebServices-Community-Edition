...
namespace CometServer.Services
{
    ...
    public sealed partial class MeasurementScaleService : ServiceBase, IMeasurementScaleService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="MeasurementScale"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        public bool UpsertConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            if (measurementScale.IsSameOrDerivedClass<IntervalScale>())
            {
                return this.IntervalScaleService.UpsertConcept(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<LogarithmicScale>())
            {
                return this.LogarithmicScaleService.UpsertConcept(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<OrdinalScale>())
            {
                return this.OrdinalScaleService.UpsertConcept(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<RatioScale>())
            {
                return this.RatioScaleService.UpsertConcept(transaction, partition, measurementScale, container);
            }
            throw new NotSupportedException(string.Format("The supplied DTO type: {0} is not a supported type", measurementScale.GetType().Name));
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="MeasurementScale"/> and contained <see cref="Thing"/>s.
        /// </returns>
