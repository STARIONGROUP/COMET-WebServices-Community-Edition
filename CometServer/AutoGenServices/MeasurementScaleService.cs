// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MeasurementScaleService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    /// <summary>
    /// The <see cref="MeasurementScale"/> Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class MeasurementScaleService : ServiceBase, IMeasurementScaleService
    {
        /// <summary>
        /// Gets or sets the <see cref="IIntervalScaleService"/>.
        /// </summary>
        public IIntervalScaleService IntervalScaleService { get; set; }

        /// <summary>
        /// Gets or sets the <see cref="ILogarithmicScaleService"/>.
        /// </summary>
        public ILogarithmicScaleService LogarithmicScaleService { get; set; }

        /// <summary>
        /// Gets or sets the <see cref="IOrdinalScaleService"/>.
        /// </summary>
        public IOrdinalScaleService OrdinalScaleService { get; set; }

        /// <summary>
        /// Gets or sets the <see cref="IRatioScaleService"/>.
        /// </summary>
        public IRatioScaleService RatioScaleService { get; set; }

        /// <summary>
        /// Get the requested <see cref="MeasurementScale"/>s from the ORM layer.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="MeasurementScale"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        /// <summary>
        /// Delete the supplied <see cref="MeasurementScale"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// The container instance of the <see cref="MeasurementScale"/> to be removed.
        /// </param>
        /// <returns>
        ...
        /// <summary>
        /// Update the supplied <see cref="MeasurementScale"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="MeasurementScale"/> <see cref="Thing"/> to update.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="MeasurementScale"/> to be updated.
        /// </param>
        /// <returns>
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            {
                return await this.IntervalScaleService.UpdateConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<LogarithmicScale>())
            {
                return await this.LogarithmicScaleService.UpdateConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<OrdinalScale>())
            {
                return await this.OrdinalScaleService.UpdateConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<RatioScale>())
            {
                return await this.RatioScaleService.UpdateConceptAsync(transaction, partition, measurementScale, container);
            }
        ...
        /// <summary>
        /// Persist the supplied <see cref="MeasurementScale"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="MeasurementScale"/> <see cref="Thing"/> to create.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="MeasurementScale"/> to be persisted.
        /// </param>
        /// <param name="sequence">
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            if (measurementScale.IsSameOrDerivedClass<IntervalScale>())
            {
                return await this.IntervalScaleService.CreateConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<LogarithmicScale>())
            {
                return await this.LogarithmicScaleService.CreateConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<OrdinalScale>())
            {
                return await this.OrdinalScaleService.CreateConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<RatioScale>())
            {
                return await this.RatioScaleService.CreateConceptAsync(transaction, partition, measurementScale, container);
            }
            throw new NotSupportedException(string.Format("The supplied DTO type: {0} is not a supported type", measurementScale.GetType().Name));
        }

        /// <summary>
        /// Persist the supplied <see cref="MeasurementScale"/> instance. Update if it already exists.
        /// This is typically used during the import of existing data to the Database.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        ...
        public async Task<bool> UpsertConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            if (measurementScale.IsSameOrDerivedClass<IntervalScale>())
            {
                return await this.IntervalScaleService.UpsertConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<LogarithmicScale>())
            {
                return await this.LogarithmicScaleService.UpsertConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<OrdinalScale>())
            {
                return await this.OrdinalScaleService.UpsertConceptAsync(transaction, partition, measurementScale, container);
            }

            if (measurementScale.IsSameOrDerivedClass<RatioScale>())
            {
                return await this.RatioScaleService.UpsertConceptAsync(transaction, partition, measurementScale, container);
            }
            throw new NotSupportedException(string.Format("The supplied DTO type: {0} is not a supported type", measurementScale.GetType().Name));
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="MeasurementScale"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            measurementScaleColl.AddRange(await this.IntervalScaleService.GetShallowAsync(transaction, partition, idFilter, authorizedContext));
            measurementScaleColl.AddRange(await this.LogarithmicScaleService.GetShallowAsync(transaction, partition, idFilter, authorizedContext));
            measurementScaleColl.AddRange(await this.OrdinalScaleService.GetShallowAsync(transaction, partition, idFilter, authorizedContext));
            measurementScaleColl.AddRange(await this.RatioScaleService.GetShallowAsync(transaction, partition, idFilter, authorizedContext));

            return await this.AfterGetAsync(measurementScaleColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="MeasurementScale"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.IntervalScaleService.GetDeepAsync(transaction, partition, idFilter, containerSecurityContext));
            results.AddRange(await this.LogarithmicScaleService.GetDeepAsync(transaction, partition, idFilter, containerSecurityContext));
            results.AddRange(await this.OrdinalScaleService.GetDeepAsync(transaction, partition, idFilter, containerSecurityContext));
            results.AddRange(await this.RatioScaleService.GetDeepAsync(transaction, partition, idFilter, containerSecurityContext));
            return results;
        }
