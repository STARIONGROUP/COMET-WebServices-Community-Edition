// --------------------------------------------------------------------------------------------------------------------
// <copyright file="StakeHolderValueMapService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class StakeHolderValueMapService : ServiceBase, IStakeHolderValueMapService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="StakeHolderValueMap"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.StakeHolderValueMapDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.StakeHolderValueMapDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.StakeHolderValueMapDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.StakeHolderValueMapDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.StakeHolderValueMapDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.StakeHolderValueMapDao.Update(transaction, partition, stakeHolderValueMap, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var stakeHolderValueMap = thing as StakeHolderValueMap;
            var createSuccesful = this.StakeHolderValueMapDao.Write(transaction, partition, stakeHolderValueMap, container);
            return createSuccesful && this.CreateContainment(transaction, partition, stakeHolderValueMap);
        }
        ...
        {
            var stakeHolderValueMap = thing as StakeHolderValueMap;
            var createSuccesful = this.StakeHolderValueMapDao.Upsert(transaction, partition, stakeHolderValueMap, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, stakeHolderValueMap);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="StakeHolderValueMap"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var stakeHolderValueMapColl = new List<Thing>(this.StakeHolderValueMapDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(stakeHolderValueMapColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="StakeHolderValueMap"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, stakeHolderValueMapColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, stakeHolderValueMapColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, stakeHolderValueMapColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, stakeHolderValueMapColl.SelectMany(x => x.HyperLink), containerSecurityContext));
            results.AddRange(this.SettingsService.GetDeep(transaction, partition, stakeHolderValueMapColl.SelectMany(x => x.Settings), containerSecurityContext));
        ...
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, StakeHolderValueMap stakeHolderValueMap)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(stakeHolderValueMap.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, stakeHolderValueMap));
            }
            ...
            {
                results.Add(this.SettingsService.CreateConcept(transaction, partition, settings, stakeHolderValueMap));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, StakeHolderValueMap stakeHolderValueMap)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(stakeHolderValueMap.Alias))
            {
                results.Add(this.AliasService.UpsertConcept(transaction, partition, alias, stakeHolderValueMap));
            }

            foreach (var attachment in this.ResolveFromRequestCache(stakeHolderValueMap.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, stakeHolderValueMap));
            }

            foreach (var definition in this.ResolveFromRequestCache(stakeHolderValueMap.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, stakeHolderValueMap));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(stakeHolderValueMap.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, stakeHolderValueMap));
            }

            foreach (var settings in this.ResolveFromRequestCache(stakeHolderValueMap.Settings))
            {
                results.Add(this.SettingsService.UpsertConcept(transaction, partition, settings, stakeHolderValueMap));
            }
