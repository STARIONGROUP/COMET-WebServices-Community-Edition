// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CategoryService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class CategoryService : ServiceBase, ICategoryService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Category"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.CategoryDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.CategoryDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.CategoryDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.CategoryDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.CategoryDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.CategoryDao.Update(transaction, partition, category, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var category = thing as Category;
            var createSuccesful = this.CategoryDao.Write(transaction, partition, category, container);
            return createSuccesful && this.CreateContainment(transaction, partition, category);
        }
        ...
        {
            var category = thing as Category;
            var createSuccesful = this.CategoryDao.Upsert(transaction, partition, category, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, category);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Category"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var categoryColl = new List<Thing>(this.CategoryDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(categoryColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Category"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, categoryColl.SelectMany(x => x.Attachment), containerSecurityContext));
            ...
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, categoryColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, Category category)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(category.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, category));
            }

            foreach (var attachment in this.ResolveFromRequestCache(category.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, category));
            }

            foreach (var definition in this.ResolveFromRequestCache(category.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, category));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(category.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, category));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, Category category)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(category.Alias))
            {
                results.Add(this.AliasService.UpsertConcept(transaction, partition, alias, category));
            }

            foreach (var attachment in this.ResolveFromRequestCache(category.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, category));
            }

            foreach (var definition in this.ResolveFromRequestCache(category.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, category));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(category.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, category));
            }
