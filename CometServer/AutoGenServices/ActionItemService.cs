...
namespace CometServer.Services
{
    ...
    public sealed partial class ActionItemService : ServiceBase, IActionItemService
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="ActionItem"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        {
            return await this.ActionItemDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.ActionItemDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.ActionItemDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.ActionItemDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.ActionItemDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.ActionItemDao.UpdateAsync(transaction, partition, actionItem, container);
        }
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var actionItem = thing as ActionItem;
            var createSuccesful = await this.ActionItemDao.WriteAsync(transaction, partition, actionItem, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, actionItem);
        }
        ...
        {
            var actionItem = thing as ActionItem;
            var createSuccesful = await this.ActionItemDao.UpsertAsync(transaction, partition, actionItem, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, actionItem);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="ActionItem"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var actionItemColl = new List<Thing>(await this.ActionItemDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(actionItemColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="ActionItem"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.ApprovedByService.GetDeepAsync(transaction, partition, actionItemColl.SelectMany(x => x.ApprovedBy), containerSecurityContext));
            results.AddRange(await this.DiscussionService.GetDeepAsync(transaction, partition, actionItemColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(await this.RelatedThingService.GetDeepAsync(transaction, partition, actionItemColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, ActionItem actionItem)
        {
            var results = new List<bool>();

            foreach (var approvedBy in this.ResolveFromRequestCache(actionItem.ApprovedBy))
            {
                results.Add(await this.ApprovedByService.CreateConceptAsync(transaction, partition, approvedBy, actionItem));
            }

            foreach (var discussion in this.ResolveFromRequestCache(actionItem.Discussion))
            {
                results.Add(await this.DiscussionService.CreateConceptAsync(transaction, partition, discussion, actionItem));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(actionItem.RelatedThing))
            {
                results.Add(await this.RelatedThingService.CreateConceptAsync(transaction, partition, relatedThing, actionItem));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, ActionItem actionItem)
        {
            var results = new List<bool>();

            foreach (var approvedBy in this.ResolveFromRequestCache(actionItem.ApprovedBy))
            {
                results.Add(await this.ApprovedByService.UpsertConceptAsync(transaction, partition, approvedBy, actionItem));
            }

            foreach (var discussion in this.ResolveFromRequestCache(actionItem.Discussion))
            {
                results.Add(await this.DiscussionService.UpsertConceptAsync(transaction, partition, discussion, actionItem));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(actionItem.RelatedThing))
            {
                results.Add(await this.RelatedThingService.UpsertConceptAsync(transaction, partition, relatedThing, actionItem));
            }
