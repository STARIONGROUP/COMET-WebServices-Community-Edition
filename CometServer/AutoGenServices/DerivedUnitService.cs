// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DerivedUnitService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    /// <summary>
    /// The <see cref="DerivedUnit"/> Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class DerivedUnitService : ServiceBase, IDerivedUnitService
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="IDerivedUnitDao"/>.
        /// </summary>
        public IDerivedUnitDao DerivedUnitDao { get; set; }

        /// <summary>
        /// Get the requested <see cref="DerivedUnit"/>s from the ORM layer.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="DerivedUnit"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="DerivedUnit"/> id that will be the source for each link table record.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.DerivedUnitDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="DerivedUnit"/> id that is the source of the link table records.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.DerivedUnitDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="DerivedUnit"/> id that is the source for the reordered link table record.
        /// </param>
        ...
        {
            return await this.DerivedUnitDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        /// <summary>
        /// Delete the supplied <see cref="DerivedUnit"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="DerivedUnit"/> to delete.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="DerivedUnit"/> to be removed.
        /// </param>
        /// <returns>
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.DerivedUnitDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.DerivedUnitDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }

        /// <summary>
        /// Update the supplied <see cref="DerivedUnit"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="DerivedUnit"/> <see cref="Thing"/> to update.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="DerivedUnit"/> to be updated.
        /// </param>
        /// <returns>
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.DerivedUnitDao.UpdateAsync(transaction, partition, derivedUnit, container);
        }

        /// <summary>
        /// Persist the supplied <see cref="DerivedUnit"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="DerivedUnit"/> <see cref="Thing"/> to create.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="DerivedUnit"/> to be persisted.
        /// </param>
        /// <param name="sequence">
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var derivedUnit = thing as DerivedUnit;
            var createSuccesful = await this.DerivedUnitDao.WriteAsync(transaction, partition, derivedUnit, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, derivedUnit);
        }

        /// <summary>
        /// Persist the supplied <see cref="DerivedUnit"/> instance. Update if it already exists.
        /// This is typically used during the import of existing data to the Database.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        ...
        {
            var derivedUnit = thing as DerivedUnit;
            var createSuccesful = await this.DerivedUnitDao.UpsertAsync(transaction, partition, derivedUnit, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, derivedUnit);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="DerivedUnit"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var authorizedContext = this.AuthorizeReadRequest("DerivedUnit", containerSecurityContext, partition);
            ...
            var derivedUnitColl = new List<Thing>(await this.DerivedUnitDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(derivedUnitColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="DerivedUnit"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.AliasService.GetDeepAsync(transaction, partition, derivedUnitColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(await this.DefinitionService.GetDeepAsync(transaction, partition, derivedUnitColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(await this.HyperLinkService.GetDeepAsync(transaction, partition, derivedUnitColl.SelectMany(x => x.HyperLink), containerSecurityContext));
            results.AddRange(await this.UnitFactorService.GetDeepAsync(transaction, partition, derivedUnitColl.SelectMany(x => x.UnitFactor).ToIdList(), containerSecurityContext));
        ...
        /// <summary>
        /// Persist the <see cref="DerivedUnit"/> containment tree to the ORM layer.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, DerivedUnit derivedUnit)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(derivedUnit.Alias))
            {
                results.Add(await this.AliasService.CreateConceptAsync(transaction, partition, alias, derivedUnit));
            }

            foreach (var definition in this.ResolveFromRequestCache(derivedUnit.Definition))
            {
                results.Add(await this.DefinitionService.CreateConceptAsync(transaction, partition, definition, derivedUnit));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(derivedUnit.HyperLink))
            {
                results.Add(await this.HyperLinkService.CreateConceptAsync(transaction, partition, hyperLink, derivedUnit));
            }

            foreach (var unitFactor in this.ResolveFromRequestCache(derivedUnit.UnitFactor))
            {
                results.Add(await this.UnitFactorService.CreateConceptAsync(transaction, partition, (UnitFactor)unitFactor.V, derivedUnit, unitFactor.K));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, DerivedUnit derivedUnit)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(derivedUnit.Alias))
            {
                results.Add(await this.AliasService.UpsertConceptAsync(transaction, partition, alias, derivedUnit));
            }

            foreach (var definition in this.ResolveFromRequestCache(derivedUnit.Definition))
            {
                results.Add(await this.DefinitionService.UpsertConceptAsync(transaction, partition, definition, derivedUnit));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(derivedUnit.HyperLink))
            {
                results.Add(await this.HyperLinkService.UpsertConceptAsync(transaction, partition, hyperLink, derivedUnit));
            }

            foreach (var unitFactor in this.ResolveFromRequestCache(derivedUnit.UnitFactor))
            {
                results.Add(await this.UnitFactorService.UpsertConceptAsync(transaction, partition, (UnitFactor)unitFactor.V, derivedUnit, unitFactor.K));
            }
