// --------------------------------------------------------------------------------------------------------------------
// <copyright file="UnitPrefixService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class UnitPrefixService : ServiceBase, IUnitPrefixService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="UnitPrefix"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.UnitPrefixDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.UnitPrefixDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.UnitPrefixDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.UnitPrefixDao.Update(transaction, partition, unitPrefix, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var unitPrefix = thing as UnitPrefix;
            var createSuccesful = this.UnitPrefixDao.Write(transaction, partition, unitPrefix, container);
            return createSuccesful && this.CreateContainment(transaction, partition, unitPrefix);
        }
        ...
        {
            var unitPrefix = thing as UnitPrefix;
            var createSuccesful = this.UnitPrefixDao.Upsert(transaction, partition, unitPrefix, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, unitPrefix);
        }
        ...
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var unitPrefixColl = new List<Thing>(this.UnitPrefixDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(unitPrefixColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="UnitPrefix"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AliasService.GetDeep(transaction, partition, unitPrefixColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, unitPrefixColl.SelectMany(x => x.Attachment), containerSecurityContext));
            results.AddRange(this.DefinitionService.GetDeep(transaction, partition, unitPrefixColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(this.HyperLinkService.GetDeep(transaction, partition, unitPrefixColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, UnitPrefix unitPrefix)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(unitPrefix.Alias))
            {
                results.Add(this.AliasService.CreateConcept(transaction, partition, alias, unitPrefix));
            }

            foreach (var attachment in this.ResolveFromRequestCache(unitPrefix.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, unitPrefix));
            }

            foreach (var definition in this.ResolveFromRequestCache(unitPrefix.Definition))
            {
                results.Add(this.DefinitionService.CreateConcept(transaction, partition, definition, unitPrefix));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(unitPrefix.HyperLink))
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, unitPrefix));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, UnitPrefix unitPrefix)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(unitPrefix.Alias))
            {
                results.Add(this.AliasService.UpsertConcept(transaction, partition, alias, unitPrefix));
            }

            foreach (var attachment in this.ResolveFromRequestCache(unitPrefix.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, unitPrefix));
            }

            foreach (var definition in this.ResolveFromRequestCache(unitPrefix.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, unitPrefix));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(unitPrefix.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, unitPrefix));
            }
