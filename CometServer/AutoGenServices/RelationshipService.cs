...
namespace CometServer.Services
{
    ...
    public sealed partial class RelationshipService : ServiceBase, IRelationshipService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Relationship"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            {
                return this.BinaryRelationshipService.UpdateConcept(transaction, partition, relationship, container);
            }

            if (relationship.IsSameOrDerivedClass<MultiRelationship>())
            {
                return this.MultiRelationshipService.UpdateConcept(transaction, partition, relationship, container);
            }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            if (relationship.IsSameOrDerivedClass<BinaryRelationship>())
            {
                return this.BinaryRelationshipService.CreateConcept(transaction, partition, relationship, container);
            }

            if (relationship.IsSameOrDerivedClass<MultiRelationship>())
            {
                return this.MultiRelationshipService.CreateConcept(transaction, partition, relationship, container);
            }
            throw new NotSupportedException(string.Format("The supplied DTO type: {0} is not a supported type", relationship.GetType().Name));
        }
        ...
        public bool UpsertConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            if (relationship.IsSameOrDerivedClass<BinaryRelationship>())
            {
                return this.BinaryRelationshipService.UpsertConcept(transaction, partition, relationship, container);
            }

            if (relationship.IsSameOrDerivedClass<MultiRelationship>())
            {
                return this.MultiRelationshipService.UpsertConcept(transaction, partition, relationship, container);
            }
            throw new NotSupportedException(string.Format("The supplied DTO type: {0} is not a supported type", relationship.GetType().Name));
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Relationship"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            relationshipColl.AddRange(this.BinaryRelationshipService.GetShallow(transaction, partition, idFilter, authorizedContext));
            relationshipColl.AddRange(this.MultiRelationshipService.GetShallow(transaction, partition, idFilter, authorizedContext));

            return this.AfterGet(relationshipColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="Relationship"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.BinaryRelationshipService.GetDeep(transaction, partition, idFilter, containerSecurityContext));
            results.AddRange(this.MultiRelationshipService.GetDeep(transaction, partition, idFilter, containerSecurityContext));
            return results;
        }
