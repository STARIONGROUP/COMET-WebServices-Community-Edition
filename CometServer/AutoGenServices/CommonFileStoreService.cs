// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CommonFileStoreService.cs" company="RHEA System S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class CommonFileStoreService : ServiceBase, ICommonFileStoreService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="CommonFileStore"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.CommonFileStoreDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.CommonFileStoreDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.CommonFileStoreDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.CommonFileStoreDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.CommonFileStoreDao.Update(transaction, partition, commonFileStore, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var commonFileStore = thing as CommonFileStore;
            var createSuccesful = this.CommonFileStoreDao.Write(transaction, partition, commonFileStore, container);
            return createSuccesful && this.CreateContainment(transaction, partition, commonFileStore);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="CommonFileStore"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var commonFileStoreColl = new List<Thing>(this.CommonFileStoreDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(commonFileStoreColl, transaction, partition, idFilter);
        }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, CommonFileStore commonFileStore)
        {
            var results = new List<bool>();

            foreach (var file in this.ResolveFromRequestCache(commonFileStore.File))
            {
                results.Add(this.FileService.CreateConcept(transaction, partition, file, commonFileStore));
            }

            foreach (var folder in this.ResolveFromRequestCache(commonFileStore.Folder))
            {
                results.Add(this.FolderService.CreateConcept(transaction, partition, folder, commonFileStore));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, CommonFileStore commonFileStore)
        {
            var results = new List<bool>();

            foreach (var file in this.ResolveFromRequestCache(commonFileStore.File))
            {
                results.Add(this.FileService.UpsertConcept(transaction, partition, file, commonFileStore));
            }

            foreach (var folder in this.ResolveFromRequestCache(commonFileStore.Folder))
            {
                results.Add(this.FolderService.UpsertConcept(transaction, partition, folder, commonFileStore));
            }
