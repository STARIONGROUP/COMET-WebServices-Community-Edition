// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GoalService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    /// <summary>
    /// The <see cref="Goal"/> Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class GoalService : ServiceBase, IGoalService
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="IGoalDao"/>.
        /// </summary>
        public IGoalDao GoalDao { get; set; }

        /// <summary>
        /// Get the requested <see cref="Goal"/>s from the ORM layer.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="Goal"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="Goal"/> id that will be the source for each link table record.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.GoalDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="Goal"/> id that is the source of the link table records.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.GoalDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="Goal"/> id that is the source for the reordered link table record.
        /// </param>
        ...
        {
            return await this.GoalDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.GoalDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.GoalDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="Goal"/> <see cref="Thing"/> to update.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="Goal"/> to be updated.
        /// </param>
        /// <returns>
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.GoalDao.UpdateAsync(transaction, partition, goal, container);
        }

        /// <summary>
        /// Persist the supplied <see cref="Goal"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="Goal"/> <see cref="Thing"/> to create.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="Goal"/> to be persisted.
        /// </param>
        /// <param name="sequence">
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var goal = thing as Goal;
            var createSuccesful = await this.GoalDao.WriteAsync(transaction, partition, goal, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, goal);
        }
        ...
        {
            var goal = thing as Goal;
            var createSuccesful = await this.GoalDao.UpsertAsync(transaction, partition, goal, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, goal);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Goal"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var authorizedContext = this.AuthorizeReadRequest("Goal", containerSecurityContext, partition);
            ...
            var goalColl = new List<Thing>(await this.GoalDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(goalColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Goal"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var goalColl = results.Where(i => i.GetType() == typeof(Goal)).Cast<Goal>().ToList();

            results.AddRange(await this.AliasService.GetDeepAsync(transaction, partition, goalColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(await this.DefinitionService.GetDeepAsync(transaction, partition, goalColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(await this.HyperLinkService.GetDeepAsync(transaction, partition, goalColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// <summary>
        /// Persist the <see cref="Goal"/> containment tree to the ORM layer.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="goal">
        /// The <see cref="Goal"/> instance to persist.
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, Goal goal)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(goal.Alias))
            {
                results.Add(await this.AliasService.CreateConceptAsync(transaction, partition, alias, goal));
            }

            foreach (var definition in this.ResolveFromRequestCache(goal.Definition))
            {
                results.Add(await this.DefinitionService.CreateConceptAsync(transaction, partition, definition, goal));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(goal.HyperLink))
            {
                results.Add(await this.HyperLinkService.CreateConceptAsync(transaction, partition, hyperLink, goal));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, Goal goal)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(goal.Alias))
            {
                results.Add(await this.AliasService.UpsertConceptAsync(transaction, partition, alias, goal));
            }

            foreach (var definition in this.ResolveFromRequestCache(goal.Definition))
            {
                results.Add(await this.DefinitionService.UpsertConceptAsync(transaction, partition, definition, goal));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(goal.HyperLink))
            {
                results.Add(await this.HyperLinkService.UpsertConceptAsync(transaction, partition, hyperLink, goal));
            }
