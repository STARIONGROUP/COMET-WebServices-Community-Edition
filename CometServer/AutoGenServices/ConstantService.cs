// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ConstantService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class ConstantService : ServiceBase, IConstantService
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="Constant"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        {
            return await this.ConstantDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.ConstantDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.ConstantDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.ConstantDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.ConstantDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.ConstantDao.UpdateAsync(transaction, partition, constant, container);
        }
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var constant = thing as Constant;
            var createSuccesful = await this.ConstantDao.WriteAsync(transaction, partition, constant, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, constant);
        }
        ...
        {
            var constant = thing as Constant;
            var createSuccesful = await this.ConstantDao.UpsertAsync(transaction, partition, constant, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, constant);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Constant"/> as result.
        /// </returns>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Constant"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.AliasService.GetDeepAsync(transaction, partition, constantColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(await this.DefinitionService.GetDeepAsync(transaction, partition, constantColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(await this.HyperLinkService.GetDeepAsync(transaction, partition, constantColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, Constant constant)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(constant.Alias))
            {
                results.Add(await this.AliasService.CreateConceptAsync(transaction, partition, alias, constant));
            }

            foreach (var definition in this.ResolveFromRequestCache(constant.Definition))
            {
                results.Add(await this.DefinitionService.CreateConceptAsync(transaction, partition, definition, constant));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(constant.HyperLink))
            {
                results.Add(await this.HyperLinkService.CreateConceptAsync(transaction, partition, hyperLink, constant));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, Constant constant)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(constant.Alias))
            {
                results.Add(await this.AliasService.UpsertConceptAsync(transaction, partition, alias, constant));
            }

            foreach (var definition in this.ResolveFromRequestCache(constant.Definition))
            {
                results.Add(await this.DefinitionService.UpsertConceptAsync(transaction, partition, definition, constant));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(constant.HyperLink))
            {
                results.Add(await this.HyperLinkService.UpsertConceptAsync(transaction, partition, hyperLink, constant));
            }
