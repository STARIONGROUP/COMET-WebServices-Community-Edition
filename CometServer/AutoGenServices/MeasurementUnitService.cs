// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MeasurementUnitService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class MeasurementUnitService : ServiceBase, IMeasurementUnitService
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="MeasurementUnit"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="MeasurementUnit"/> id that will be the source for each link table record.
        /// </param>
        /// <param name="value">
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="MeasurementUnit"/> id that is the source of the link table records.
        /// </param>
        /// <param name="value">
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="MeasurementUnit"/> id that is the source for the reordered link table record.
        /// </param>
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            {
                return await this.ConversionBasedUnitService.UpdateConceptAsync(transaction, partition, measurementUnit, container);
            }

            if (measurementUnit.IsSameOrDerivedClass<DerivedUnit>())
            {
                return await this.DerivedUnitService.UpdateConceptAsync(transaction, partition, measurementUnit, container);
            }

            if (measurementUnit.IsSameOrDerivedClass<SimpleUnit>())
            {
                return await this.SimpleUnitService.UpdateConceptAsync(transaction, partition, measurementUnit, container);
            }
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            if (measurementUnit.IsSameOrDerivedClass<ConversionBasedUnit>())
            {
                return await this.ConversionBasedUnitService.CreateConceptAsync(transaction, partition, measurementUnit, container);
            }

            if (measurementUnit.IsSameOrDerivedClass<DerivedUnit>())
            {
                return await this.DerivedUnitService.CreateConceptAsync(transaction, partition, measurementUnit, container);
            }

            if (measurementUnit.IsSameOrDerivedClass<SimpleUnit>())
            {
                return await this.SimpleUnitService.CreateConceptAsync(transaction, partition, measurementUnit, container);
            }
            throw new NotSupportedException(string.Format("The supplied DTO type: {0} is not a supported type", measurementUnit.GetType().Name));
        }
        ...
        public async Task<bool> UpsertConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            if (measurementUnit.IsSameOrDerivedClass<ConversionBasedUnit>())
            {
                return await this.ConversionBasedUnitService.UpsertConceptAsync(transaction, partition, measurementUnit, container);
            }

            if (measurementUnit.IsSameOrDerivedClass<DerivedUnit>())
            {
                return await this.DerivedUnitService.UpsertConceptAsync(transaction, partition, measurementUnit, container);
            }

            if (measurementUnit.IsSameOrDerivedClass<SimpleUnit>())
            {
                return await this.SimpleUnitService.UpsertConceptAsync(transaction, partition, measurementUnit, container);
            }
            throw new NotSupportedException(string.Format("The supplied DTO type: {0} is not a supported type", measurementUnit.GetType().Name));
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="MeasurementUnit"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var authorizedContext = this.AuthorizeReadRequest("MeasurementUnit", containerSecurityContext, partition);
            ...
            measurementUnitColl.AddRange(await this.ConversionBasedUnitService.GetShallowAsync(transaction, partition, idFilter, authorizedContext));
            measurementUnitColl.AddRange(await this.DerivedUnitService.GetShallowAsync(transaction, partition, idFilter, authorizedContext));
            measurementUnitColl.AddRange(await this.SimpleUnitService.GetShallowAsync(transaction, partition, idFilter, authorizedContext));

            return await this.AfterGetAsync(measurementUnitColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="MeasurementUnit"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.ConversionBasedUnitService.GetDeepAsync(transaction, partition, idFilter, containerSecurityContext));
            results.AddRange(await this.DerivedUnitService.GetDeepAsync(transaction, partition, idFilter, containerSecurityContext));
            results.AddRange(await this.SimpleUnitService.GetDeepAsync(transaction, partition, idFilter, containerSecurityContext));
            return results;
        }
