// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RuleVerificationListService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class RuleVerificationListService : ServiceBase, IRuleVerificationListService
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="RuleVerificationList"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        {
            return await this.RuleVerificationListDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.RuleVerificationListDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.RuleVerificationListDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.RuleVerificationListDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.RuleVerificationListDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.RuleVerificationListDao.UpdateAsync(transaction, partition, ruleVerificationList, container);
        }
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var ruleVerificationList = thing as RuleVerificationList;
            var createSuccesful = await this.RuleVerificationListDao.WriteAsync(transaction, partition, ruleVerificationList, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, ruleVerificationList);
        }
        ...
        {
            var ruleVerificationList = thing as RuleVerificationList;
            var createSuccesful = await this.RuleVerificationListDao.UpsertAsync(transaction, partition, ruleVerificationList, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, ruleVerificationList);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="RuleVerificationList"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var ruleVerificationListColl = new List<Thing>(await this.RuleVerificationListDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(ruleVerificationListColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="RuleVerificationList"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.AliasService.GetDeepAsync(transaction, partition, ruleVerificationListColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(await this.DefinitionService.GetDeepAsync(transaction, partition, ruleVerificationListColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(await this.HyperLinkService.GetDeepAsync(transaction, partition, ruleVerificationListColl.SelectMany(x => x.HyperLink), containerSecurityContext));
            results.AddRange(await this.RuleVerificationService.GetDeepAsync(transaction, partition, ruleVerificationListColl.SelectMany(x => x.RuleVerification).ToIdList(), containerSecurityContext));
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, RuleVerificationList ruleVerificationList)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(ruleVerificationList.Alias))
            {
                results.Add(await this.AliasService.CreateConceptAsync(transaction, partition, alias, ruleVerificationList));
            }

            foreach (var definition in this.ResolveFromRequestCache(ruleVerificationList.Definition))
            {
                results.Add(await this.DefinitionService.CreateConceptAsync(transaction, partition, definition, ruleVerificationList));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(ruleVerificationList.HyperLink))
            {
                results.Add(await this.HyperLinkService.CreateConceptAsync(transaction, partition, hyperLink, ruleVerificationList));
            }

            foreach (var ruleVerification in this.ResolveFromRequestCache(ruleVerificationList.RuleVerification))
            {
                results.Add(await this.RuleVerificationService.CreateConceptAsync(transaction, partition, (RuleVerification)ruleVerification.V, ruleVerificationList, ruleVerification.K));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, RuleVerificationList ruleVerificationList)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(ruleVerificationList.Alias))
            {
                results.Add(await this.AliasService.UpsertConceptAsync(transaction, partition, alias, ruleVerificationList));
            }

            foreach (var definition in this.ResolveFromRequestCache(ruleVerificationList.Definition))
            {
                results.Add(await this.DefinitionService.UpsertConceptAsync(transaction, partition, definition, ruleVerificationList));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(ruleVerificationList.HyperLink))
            {
                results.Add(await this.HyperLinkService.UpsertConceptAsync(transaction, partition, hyperLink, ruleVerificationList));
            }

            foreach (var ruleVerification in this.ResolveFromRequestCache(ruleVerificationList.RuleVerification))
            {
                results.Add(await this.RuleVerificationService.UpsertConceptAsync(transaction, partition, (RuleVerification)ruleVerification.V, ruleVerificationList, ruleVerification.K));
            }
