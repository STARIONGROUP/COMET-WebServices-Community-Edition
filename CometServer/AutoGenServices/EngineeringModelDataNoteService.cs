...
namespace CometServer.Services
{
    ...
    public sealed partial class EngineeringModelDataNoteService : ServiceBase, IEngineeringModelDataNoteService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="EngineeringModelDataNote"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.EngineeringModelDataNoteDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.EngineeringModelDataNoteDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.EngineeringModelDataNoteDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.EngineeringModelDataNoteDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.EngineeringModelDataNoteDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.EngineeringModelDataNoteDao.Update(transaction, partition, engineeringModelDataNote, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var engineeringModelDataNote = thing as EngineeringModelDataNote;
            var createSuccesful = this.EngineeringModelDataNoteDao.Write(transaction, partition, engineeringModelDataNote, container);
            return createSuccesful && this.CreateContainment(transaction, partition, engineeringModelDataNote);
        }
        ...
        {
            var engineeringModelDataNote = thing as EngineeringModelDataNote;
            var createSuccesful = this.EngineeringModelDataNoteDao.Upsert(transaction, partition, engineeringModelDataNote, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, engineeringModelDataNote);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="EngineeringModelDataNote"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var engineeringModelDataNoteColl = new List<Thing>(this.EngineeringModelDataNoteDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(engineeringModelDataNoteColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="EngineeringModelDataNote"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.DiscussionService.GetDeep(transaction, partition, engineeringModelDataNoteColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(this.RelatedThingService.GetDeep(transaction, partition, engineeringModelDataNoteColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
        ...
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, EngineeringModelDataNote engineeringModelDataNote)
        {
            ...
            {
                results.Add(this.DiscussionService.CreateConcept(transaction, partition, discussion, engineeringModelDataNote));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(engineeringModelDataNote.RelatedThing))
            {
                results.Add(this.RelatedThingService.CreateConcept(transaction, partition, relatedThing, engineeringModelDataNote));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, EngineeringModelDataNote engineeringModelDataNote)
        {
            var results = new List<bool>();

            foreach (var discussion in this.ResolveFromRequestCache(engineeringModelDataNote.Discussion))
            {
                results.Add(this.DiscussionService.UpsertConcept(transaction, partition, discussion, engineeringModelDataNote));
            }

            foreach (var relatedThing in this.ResolveFromRequestCache(engineeringModelDataNote.RelatedThing))
            {
                results.Add(this.RelatedThingService.UpsertConcept(transaction, partition, relatedThing, engineeringModelDataNote));
            }
