// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PossibleFiniteStateService.cs" company="Starion Group S.A.">
...
namespace CometServer.Services
{
    ...
    public sealed partial class PossibleFiniteStateService : ServiceBase, IPossibleFiniteStateService
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="PossibleFiniteState"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        {
            return await this.PossibleFiniteStateDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.PossibleFiniteStateDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.PossibleFiniteStateDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        {
            return await this.PossibleFiniteStateDao.ReorderContainmentAsync(transaction, partition, orderedItem);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.PossibleFiniteStateDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.PossibleFiniteStateDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.PossibleFiniteStateDao.UpdateAsync(transaction, partition, possibleFiniteState, container);
        }
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var possibleFiniteState = thing as PossibleFiniteState;
            var createSuccesful = await this.PossibleFiniteStateDao.WriteAsync(transaction, partition, possibleFiniteState, sequence, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, possibleFiniteState);
        }
        ...
        {
            var possibleFiniteState = thing as PossibleFiniteState;
            var createSuccesful = await this.PossibleFiniteStateDao.UpsertAsync(transaction, partition, possibleFiniteState, sequence, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, possibleFiniteState);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="PossibleFiniteState"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var possibleFiniteStateColl = new List<Thing>(await this.PossibleFiniteStateDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(possibleFiniteStateColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="PossibleFiniteState"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.AliasService.GetDeepAsync(transaction, partition, possibleFiniteStateColl.SelectMany(x => x.Alias), containerSecurityContext));
            results.AddRange(await this.DefinitionService.GetDeepAsync(transaction, partition, possibleFiniteStateColl.SelectMany(x => x.Definition), containerSecurityContext));
            results.AddRange(await this.HyperLinkService.GetDeepAsync(transaction, partition, possibleFiniteStateColl.SelectMany(x => x.HyperLink), containerSecurityContext));
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, PossibleFiniteState possibleFiniteState)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(possibleFiniteState.Alias))
            {
                results.Add(await this.AliasService.CreateConceptAsync(transaction, partition, alias, possibleFiniteState));
            }

            foreach (var definition in this.ResolveFromRequestCache(possibleFiniteState.Definition))
            {
                results.Add(await this.DefinitionService.CreateConceptAsync(transaction, partition, definition, possibleFiniteState));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(possibleFiniteState.HyperLink))
            {
                results.Add(await this.HyperLinkService.CreateConceptAsync(transaction, partition, hyperLink, possibleFiniteState));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, PossibleFiniteState possibleFiniteState)
        {
            var results = new List<bool>();

            foreach (var alias in this.ResolveFromRequestCache(possibleFiniteState.Alias))
            {
                results.Add(await this.AliasService.UpsertConceptAsync(transaction, partition, alias, possibleFiniteState));
            }

            foreach (var definition in this.ResolveFromRequestCache(possibleFiniteState.Definition))
            {
                results.Add(await this.DefinitionService.UpsertConceptAsync(transaction, partition, definition, possibleFiniteState));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(possibleFiniteState.HyperLink))
            {
                results.Add(await this.HyperLinkService.UpsertConceptAsync(transaction, partition, hyperLink, possibleFiniteState));
            }
