...
namespace CometServer.Services
{
    ...
    public sealed partial class OrdinalScaleService : ServiceBase, IOrdinalScaleService
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="OrdinalScale"/>, optionally with contained <see cref="Thing"/>s.
        /// </returns>
        ...
        {
            return this.OrdinalScaleDao.AddToCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.OrdinalScaleDao.DeleteFromCollectionProperty(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return this.OrdinalScaleDao.ReorderCollectionProperty(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        public bool DeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.OrdinalScaleDao.Delete(transaction, partition, thing.Iid);
        }
        ...
        public bool RawDeleteConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return this.OrdinalScaleDao.RawDelete(transaction, partition, thing.Iid);
        }
        ...
        public bool UpdateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return this.OrdinalScaleDao.Update(transaction, partition, ordinalScale, container);
        }
        ...
        public bool CreateConcept(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var ordinalScale = thing as OrdinalScale;
            var createSuccesful = this.OrdinalScaleDao.Write(transaction, partition, ordinalScale, container);
            return createSuccesful && this.CreateContainment(transaction, partition, ordinalScale);
        }
        ...
        {
            var ordinalScale = thing as OrdinalScale;
            var createSuccesful = this.OrdinalScaleDao.Upsert(transaction, partition, ordinalScale, container);
            return createSuccesful && this.UpsertContainment(transaction, partition, ordinalScale);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="OrdinalScale"/>.
        /// </returns>
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var ordinalScaleColl = new List<Thing>(this.OrdinalScaleDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction), (DateTime)this.TransactionManager.GetRawSessionInstant(transaction)));

            return this.AfterGet(ordinalScaleColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="OrdinalScale"/> and contained <see cref="Thing"/>s.
        /// </returns>
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.ValueDefinitionService.GetDeep(transaction, partition, ordinalScaleColl.SelectMany(x => x.ValueDefinition), containerSecurityContext));
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, OrdinalScale ordinalScale)
        {
            ...
            {
                results.Add(this.HyperLinkService.CreateConcept(transaction, partition, hyperLink, ordinalScale));
            }

            foreach (var mappingToReferenceScale in this.ResolveFromRequestCache(ordinalScale.MappingToReferenceScale))
            {
                results.Add(this.MappingToReferenceScaleService.CreateConcept(transaction, partition, mappingToReferenceScale, ordinalScale));
            }

            foreach (var valueDefinition in this.ResolveFromRequestCache(ordinalScale.ValueDefinition))
            {
                results.Add(this.ValueDefinitionService.CreateConcept(transaction, partition, valueDefinition, ordinalScale));
            }
        ...
        /// True if the persistence was successful.
        /// </returns>
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, OrdinalScale ordinalScale)
        {
            var results = new List<bool>();
            ...
            foreach (var definition in this.ResolveFromRequestCache(ordinalScale.Definition))
            {
                results.Add(this.DefinitionService.UpsertConcept(transaction, partition, definition, ordinalScale));
            }

            foreach (var hyperLink in this.ResolveFromRequestCache(ordinalScale.HyperLink))
            {
                results.Add(this.HyperLinkService.UpsertConcept(transaction, partition, hyperLink, ordinalScale));
            }

            foreach (var mappingToReferenceScale in this.ResolveFromRequestCache(ordinalScale.MappingToReferenceScale))
            {
                results.Add(this.MappingToReferenceScaleService.UpsertConcept(transaction, partition, mappingToReferenceScale, ordinalScale));
            }

            foreach (var valueDefinition in this.ResolveFromRequestCache(ordinalScale.ValueDefinition))
            {
                results.Add(this.ValueDefinitionService.UpsertConcept(transaction, partition, valueDefinition, ordinalScale));
            }
