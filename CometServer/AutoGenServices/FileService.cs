...
namespace CometServer.Services
{
    ...
    /// </summary>
    public sealed partial class FileService : ServiceBase, IFileService
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the <see cref="IFileDao"/>.
        /// </summary>
        public IFileDao FileDao { get; set; }

        /// <summary>
        /// Get the requested <see cref="File"/>s from the ORM layer.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="File"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="File"/> id that will be the source for each link table record.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.FileDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="File"/> id that is the source of the link table records.
        /// </param>
        /// <param name="value">
        ...
        {
            return await this.FileDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="File"/> id that is the source for the reordered link table record.
        /// </param>
        ...
        {
            return await this.FileDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        /// <summary>
        /// Delete the supplied <see cref="File"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="File"/> to delete.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="File"/> to be removed.
        /// </param>
        /// <returns>
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.FileDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.FileDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }

        /// <summary>
        /// Update the supplied <see cref="File"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="File"/> <see cref="Thing"/> to update.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="File"/> to be updated.
        /// </param>
        /// <returns>
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.FileDao.UpdateAsync(transaction, partition, file, container);
        }

        /// <summary>
        /// Persist the supplied <see cref="File"/> instance.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="thing">
        /// The <see cref="File"/> <see cref="Thing"/> to create.
        /// </param>
        /// <param name="container">
        /// The container instance of the <see cref="File"/> to be persisted.
        /// </param>
        /// <param name="sequence">
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var file = thing as File;
            var createSuccesful = await this.FileDao.WriteAsync(transaction, partition, file, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, file);
        }

        /// <summary>
        /// Persist the supplied <see cref="File"/> instance. Update if it already exists.
        /// This is typically used during the import of existing data to the Database.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        ...
        {
            var file = thing as File;
            var createSuccesful = await this.FileDao.UpsertAsync(transaction, partition, file, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, file);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="File"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var authorizedContext = this.AuthorizeReadRequest("File", containerSecurityContext, partition);
            ...
            var fileColl = new List<Thing>(await this.FileDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(fileColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="File"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var fileColl = results.Where(i => i.GetType() == typeof(File)).Cast<File>().ToList();

            results.AddRange(await this.FileRevisionService.GetDeepAsync(transaction, partition, fileColl.SelectMany(x => x.FileRevision), containerSecurityContext));
        ...
        /// <summary>
        /// Persist the <see cref="File"/> containment tree to the ORM layer.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        /// </param>
        ...
        /// </param>
        /// <param name="file">
        /// The <see cref="File"/> instance to persist.
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, File file)
        {
            var results = new List<bool>();

            foreach (var fileRevision in this.ResolveFromRequestCache(file.FileRevision))
            {
                results.Add(await this.FileRevisionService.CreateConceptAsync(transaction, partition, fileRevision, file));
            }
        ...
        /// <summary>
        /// Persist the <see cref="File"/> containment tree to the ORM layer. Update if it already exists.
        /// This is typically used during the import of existing data to the Database.
        /// </summary>
        /// <param name="transaction">
        /// The current <see cref="NpgsqlTransaction"/> to the database.
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, File file)
        {
            var results = new List<bool>();

            foreach (var fileRevision in this.ResolveFromRequestCache(file.FileRevision))
            {
                results.Add(await this.FileRevisionService.UpsertConceptAsync(transaction, partition, fileRevision, file));
            }
