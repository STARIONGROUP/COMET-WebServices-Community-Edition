...
namespace CometServer.Services
{
    ...
    public sealed partial class SectionService : ServiceBase, ISectionService
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="Section"/>, optionally with contained <see cref="Thing"/>s as result.
        /// </returns>
        ...
        {
            return await this.SectionDao.AddToCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.SectionDao.DeleteFromCollectionPropertyAsync(transaction, partition, propertyName, iid, value);
        }
        ...
        {
            return await this.SectionDao.ReorderCollectionPropertyAsync(transaction, partition, propertyName, iid, orderUpdate);
        }
        ...
        {
            return await this.SectionDao.ReorderContainmentAsync(transaction, partition, orderedItem);
        }
        ...
        public async Task<bool> DeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.SectionDao.DeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> RawDeleteConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container = null)
        {
            ...
            return await this.SectionDao.RawDeleteAsync(transaction, partition, thing.Iid);
        }
        ...
        public async Task<bool> UpdateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container)
        {
            ...
            return await this.SectionDao.UpdateAsync(transaction, partition, section, container);
        }
        ...
        public async Task<bool> CreateConceptAsync(NpgsqlTransaction transaction, string partition, Thing thing, Thing container, long sequence = -1)
        {
            ...
            var section = thing as Section;
            var createSuccesful = await this.SectionDao.WriteAsync(transaction, partition, section, sequence, container);
            return createSuccesful && await this.CreateContainmentAsync(transaction, partition, section);
        }
        ...
        {
            var section = thing as Section;
            var createSuccesful = await this.SectionDao.UpsertAsync(transaction, partition, section, sequence, container);
            return createSuccesful && await this.UpsertContainmentAsync(transaction, partition, section);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Section"/> as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetShallowAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var sectionColl = new List<Thing>(await this.SectionDao.ReadAsync(transaction, partition, idFilter, await this.TransactionManager.IsCachedDtoReadEnabledAsync(transaction), (DateTime)(await this.TransactionManager.GetRawSessionInstantAsync(transaction))));

            return await this.AfterGetAsync(sectionColl, transaction, partition, idFilter);
        }
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having List of instances of <see cref="Section"/> and contained <see cref="Thing"/>s as result.
        /// </returns>
        public async Task<IEnumerable<Thing>> GetDeepAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(await this.PageService.GetDeepAsync(transaction, partition, sectionColl.SelectMany(x => x.Page).ToIdList(), containerSecurityContext));
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> CreateContainmentAsync(NpgsqlTransaction transaction, string partition, Section section)
        {
            var results = new List<bool>();

            foreach (var page in this.ResolveFromRequestCache(section.Page))
            {
                results.Add(await this.PageService.CreateConceptAsync(transaction, partition, (Page)page.V, section, page.K));
            }
        ...
        /// An awaitable <see cref="Task"/> having True if the persistence was successful as result.
        /// </returns>
        private async Task<bool> UpsertContainmentAsync(NpgsqlTransaction transaction, string partition, Section section)
        {
            var results = new List<bool>();

            foreach (var page in this.ResolveFromRequestCache(section.Page))
            {
                results.Add(await this.PageService.UpsertConceptAsync(transaction, partition, (Page)page.V, section, page.K));
            }
