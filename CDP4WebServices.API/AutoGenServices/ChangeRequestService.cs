...
namespace CDP4WebServices.API.Services
{
    ...
    /// <summary>
    /// The ChangeRequest Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class ChangeRequestService : ServiceBase, IChangeRequestService
    {
        ...
        /// <summary>
        /// Gets or sets the changeRequest dao.
        /// </summary>
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.ModellingThingReferenceService.GetDeep(transaction, partition, changeRequestColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
            results.AddRange(this.EngineeringModelDataDiscussionItemService.GetDeep(transaction, partition, changeRequestColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(this.ApprovalService.GetDeep(transaction, partition, changeRequestColl.SelectMany(x => x.ApprovedBy), containerSecurityContext));
        ...
        /// The changeRequest instance to persist.
        /// </param>
        /// <returns>
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, ChangeRequest changeRequest)
        {
            ...
            {
               results.Add(this.ModellingThingReferenceService.CreateConcept(transaction, partition, relatedThing, changeRequest));
            }

            foreach (var discussion in this.ResolveFromRequestCache(changeRequest.Discussion))
            {
               results.Add(this.EngineeringModelDataDiscussionItemService.CreateConcept(transaction, partition, discussion, changeRequest));
            }

            foreach (var approvedBy in this.ResolveFromRequestCache(changeRequest.ApprovedBy))
            {
               results.Add(this.ApprovalService.CreateConcept(transaction, partition, approvedBy, changeRequest));
            }
