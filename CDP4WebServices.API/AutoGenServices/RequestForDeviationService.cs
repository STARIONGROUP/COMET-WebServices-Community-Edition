...
namespace CDP4WebServices.API.Services
{
    ...
    /// <summary>
    /// The RequestForDeviation Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class RequestForDeviationService : ServiceBase, IRequestForDeviationService
    {
        ...
        /// <summary>
        /// Gets or sets the requestForDeviation dao.
        /// </summary>
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.ModellingThingReferenceService.GetDeep(transaction, partition, requestForDeviationColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
            results.AddRange(this.EngineeringModelDataDiscussionItemService.GetDeep(transaction, partition, requestForDeviationColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(this.ApprovalService.GetDeep(transaction, partition, requestForDeviationColl.SelectMany(x => x.ApprovedBy), containerSecurityContext));
        ...
        /// The requestForDeviation instance to persist.
        /// </param>
        /// <returns>
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, RequestForDeviation requestForDeviation)
        {
            ...
            {
               results.Add(this.ModellingThingReferenceService.CreateConcept(transaction, partition, relatedThing, requestForDeviation));
            }

            foreach (var discussion in this.ResolveFromRequestCache(requestForDeviation.Discussion))
            {
               results.Add(this.EngineeringModelDataDiscussionItemService.CreateConcept(transaction, partition, discussion, requestForDeviation));
            }

            foreach (var approvedBy in this.ResolveFromRequestCache(requestForDeviation.ApprovedBy))
            {
               results.Add(this.ApprovalService.CreateConcept(transaction, partition, approvedBy, requestForDeviation));
            }
