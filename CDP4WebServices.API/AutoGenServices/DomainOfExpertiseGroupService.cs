// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DomainOfExpertiseGroupService.cs" company="RHEA System S.A.">
...
namespace CDP4WebServices.API.Services
{
    ...
    public sealed partial class DomainOfExpertiseGroupService : ServiceBase, IDomainOfExpertiseGroupService
    {
        ...
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var domainOfExpertiseGroupColl = new List<Thing>(this.DomainOfExpertiseGroupDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction)));
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.AttachmentService.GetDeep(transaction, partition, domainOfExpertiseGroupColl.SelectMany(x => x.Attachment), containerSecurityContext));
        ...
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, DomainOfExpertiseGroup domainOfExpertiseGroup)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(domainOfExpertiseGroup.Attachment))
            {
                results.Add(this.AttachmentService.CreateConcept(transaction, partition, attachment, domainOfExpertiseGroup));
            }
        ...
        private bool UpsertContainment(NpgsqlTransaction transaction, string partition, DomainOfExpertiseGroup domainOfExpertiseGroup)
        {
            ...
            foreach (var attachment in this.ResolveFromRequestCache(domainOfExpertiseGroup.Attachment))
            {
                results.Add(this.AttachmentService.UpsertConcept(transaction, partition, attachment, domainOfExpertiseGroup));
            }
