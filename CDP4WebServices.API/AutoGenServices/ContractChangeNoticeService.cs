...
namespace CDP4WebServices.API.Services
{
    ...
    /// <summary>
    /// The ContractChangeNotice Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class ContractChangeNoticeService : ServiceBase, IContractChangeNoticeService
    {
        ...
        /// <summary>
        /// Gets or sets the contractChangeNotice dao.
        /// </summary>
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.ModellingThingReferenceService.GetDeep(transaction, partition, contractChangeNoticeColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
            results.AddRange(this.EngineeringModelDataDiscussionItemService.GetDeep(transaction, partition, contractChangeNoticeColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(this.ApprovalService.GetDeep(transaction, partition, contractChangeNoticeColl.SelectMany(x => x.ApprovedBy), containerSecurityContext));
