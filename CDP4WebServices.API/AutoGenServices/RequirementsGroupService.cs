...
namespace CDP4WebServices.API.Services
{
    ...
    /// <summary>
    /// The RequirementsGroup Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class RequirementsGroupService : ServiceBase, IRequirementsGroupService
    {
        ...
        /// <summary>
        /// Gets or sets the requirementsGroup dao.
        /// </summary>
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.GetDeep(transaction, partition, requirementsGroupColl.SelectMany(x => x.Group), containerSecurityContext));
            results.AddRange(this.RequirementsContainerParameterValueService.GetDeep(transaction, partition, requirementsGroupColl.SelectMany(x => x.ParameterValue), containerSecurityContext));
        ...
        /// The requirementsGroup instance to persist.
        /// </param>
        /// <returns>
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, RequirementsGroup requirementsGroup)
        {
            ...
            {
               results.Add(this.CreateConcept(transaction, partition, group, requirementsGroup));
            }

            foreach (var parameterValue in this.ResolveFromRequestCache(requirementsGroup.ParameterValue))
            {
               results.Add(this.RequirementsContainerParameterValueService.CreateConcept(transaction, partition, parameterValue, requirementsGroup));
            }
