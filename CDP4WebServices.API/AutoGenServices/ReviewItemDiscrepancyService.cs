...
namespace CDP4WebServices.API.Services
{
    ...
    /// <summary>
    /// The ReviewItemDiscrepancy Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class ReviewItemDiscrepancyService : ServiceBase, IReviewItemDiscrepancyService
    {
        ...
        /// <summary>
        /// Gets or sets the reviewItemDiscrepancy dao.
        /// </summary>
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.ModellingThingReferenceService.GetDeep(transaction, partition, reviewItemDiscrepancyColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
            results.AddRange(this.EngineeringModelDataDiscussionItemService.GetDeep(transaction, partition, reviewItemDiscrepancyColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(this.ApprovalService.GetDeep(transaction, partition, reviewItemDiscrepancyColl.SelectMany(x => x.ApprovedBy), containerSecurityContext));
