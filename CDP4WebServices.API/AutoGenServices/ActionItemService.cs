// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ActionItemService.cs" company="RHEA System S.A.">
...
namespace CDP4WebServices.API.Services
{
    ...
    /// <summary>
    /// The ActionItem Service which uses the ORM layer to interact with the data model.
    /// </summary>
    public sealed partial class ActionItemService : ServiceBase, IActionItemService
    {
        ...
        /// <summary>
        /// Gets or sets the actionItem dao.
        /// </summary>
        ...
        public IEnumerable<Thing> GetShallow(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            var actionItemColl = new List<Thing>(this.ActionItemDao.Read(transaction, partition, idFilter, this.TransactionManager.IsCachedDtoReadEnabled(transaction)));
        ...
        public IEnumerable<Thing> GetDeep(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids, ISecurityContext containerSecurityContext)
        {
            ...
            results.AddRange(this.ModellingThingReferenceService.GetDeep(transaction, partition, actionItemColl.SelectMany(x => x.RelatedThing), containerSecurityContext));
            results.AddRange(this.EngineeringModelDataDiscussionItemService.GetDeep(transaction, partition, actionItemColl.SelectMany(x => x.Discussion), containerSecurityContext));
            results.AddRange(this.ApprovalService.GetDeep(transaction, partition, actionItemColl.SelectMany(x => x.ApprovedBy), containerSecurityContext));
        ...
        /// The actionItem instance to persist.
        /// </param>
        /// <returns>
        /// True if the persistence was successful.
        /// </returns>
        private bool CreateContainment(NpgsqlTransaction transaction, string partition, ActionItem actionItem)
        {
            ...
            {
               results.Add(this.ModellingThingReferenceService.CreateConcept(transaction, partition, relatedThing, actionItem));
            }

            foreach (var discussion in this.ResolveFromRequestCache(actionItem.Discussion))
            {
               results.Add(this.EngineeringModelDataDiscussionItemService.CreateConcept(transaction, partition, discussion, actionItem));
            }

            foreach (var approvedBy in this.ResolveFromRequestCache(actionItem.ApprovedBy))
            {
               results.Add(this.ApprovalService.CreateConcept(transaction, partition, approvedBy, actionItem));
            }
