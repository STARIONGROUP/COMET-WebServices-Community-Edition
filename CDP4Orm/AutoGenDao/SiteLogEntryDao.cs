...
namespace CDP4Orm.Dao
{
    ...
    public partial class SiteLogEntryDao : ThingDao, ISiteLogEntryDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.SiteLogEntry"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.SiteLogEntry>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<SiteLogEntry>();
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as SiteLogEntry);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.SiteLogEntry siteLogEntry, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, siteLogEntry, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, siteLogEntry, container);
                ...
                foreach (var item in siteLogEntry.AffectedDomainIid)
                {
                    await this.AddAffectedDomainIidAsync(transaction, partition, siteLogEntry.Iid, item);
                }

                foreach (var item in siteLogEntry.AffectedItemIid)
                {
                    await this.AddAffectedItemIidAsync(transaction, partition, siteLogEntry.Iid, item);
                }

                foreach (var item in siteLogEntry.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, siteLogEntry.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, siteLogEntry, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.SiteLogEntry siteLogEntry, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, siteLogEntry, container);
            ...
            foreach (var item in siteLogEntry.AffectedDomainIid)
            {
                await this.UpsertAffectedDomainIidAsync(transaction, partition, siteLogEntry.Iid, item);
            }

            foreach (var item in siteLogEntry.AffectedItemIid)
            {
                await this.UpsertAffectedItemIidAsync(transaction, partition, siteLogEntry.Iid, item);
            }

            foreach (var item in siteLogEntry.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, siteLogEntry.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.SiteLogEntry siteLogEntry, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, siteLogEntry, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, siteLogEntry, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, siteLogEntry, container);
        }
