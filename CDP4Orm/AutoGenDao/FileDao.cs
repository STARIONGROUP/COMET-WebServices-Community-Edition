// --------------------------------------------------------------------------------------------------------------------
// <copyright file="FileDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    /// <summary>
    /// The File Data Access Object which acts as an ORM layer to the SQL database.
    /// </summary>
    [ExcludeFromCodeCoverage]
    public partial class FileDao : ThingDao, IFileDao
    {
        /// <summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.File"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.File>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"File_Cache\"", partition);
        ...
        /// </param>
        /// <returns>
        /// A deserialized instance of <see cref="CDP4Common.DTO.File"/>.
        /// </returns>
        public virtual CDP4Common.DTO.File MapToDto(NpgsqlDataReader reader)
        {
            ...
            var dto = new CDP4Common.DTO.File(iid, revisionNumber);
            ...
            dto.FileRevision.AddRange(Array.ConvertAll((string[])reader["FileRevision"], Guid.Parse));
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.File file, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, file, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, file, container);
                ...
                foreach (var item in file.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, file.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, file, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.File file, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, file, container);
            ...
            foreach (var item in file.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, file.Iid, item);
            }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.File"/> id that will be the source for each link table record.
        /// </param> 
        /// <param name="value">
        ...
        /// </param>
        /// <param name="file">
        /// The File DTO that is to be updated.
        /// </param>
        /// <param name="container">
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.File file, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, file, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, file, container);

                await using (var command = new NpgsqlCommand())
                {
                    var sqlBuilder = new System.Text.StringBuilder();
                    sqlBuilder.AppendFormat("UPDATE \"{0}\".\"File\"", partition);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, file, container);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.File"/> id that is to be deleted.
        /// </param>
        /// <returns>
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.File"/> id that is the source of each link table record.
        /// </param> 
        /// <param name="value">
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.File"/> id that is the source for each link table record.
        /// </param> 
        /// <param name="category">
        ...
        public async Task<bool> DeleteCategoryAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid category)
        {
            ...
            {
                var sqlBuilder = new System.Text.StringBuilder();
                sqlBuilder.AppendFormat("DELETE FROM \"{0}\".\"File_Category\"", partition);
                sqlBuilder.Append(" WHERE \"File\" = :file");
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="FileDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"File\".\"Container\",");

            sqlBuilder.Append(" NULL::bigint AS \"Sequence\",");

            sqlBuilder.Append(" \"Actor\",");

            sqlBuilder.Append(" \"File\".\"LockedBy\",");

            sqlBuilder.Append(" \"File\".\"Owner\",");
            sqlBuilder.Append(" COALESCE(\"Thing_ExcludedDomain\".\"ExcludedDomain\",'{}'::text[]) AS \"ExcludedDomain\",");
            sqlBuilder.Append(" COALESCE(\"Thing_ExcludedPerson\".\"ExcludedPerson\",'{}'::text[]) AS \"ExcludedPerson\",");
            sqlBuilder.Append(" COALESCE(\"File_Category\".\"Category\",'{}'::text[]) AS \"Category\",");
            sqlBuilder.Append(" COALESCE(\"File_FileRevision\".\"FileRevision\",'{}'::text[]) AS \"FileRevision\",");

            sqlBuilder.Remove(sqlBuilder.Length - 1, 1);
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Thing\"", this.GetThingDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"File\" USING (\"Iid\")", this.GetFileDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"File\" AS \"Iid\", array_agg(\"Category\"::text) AS \"Category\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"File_Category\"", this.GetFile_CategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"File\" ON \"File\" = \"Iid\"", this.GetFileDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"File\") AS \"File_Category\" USING (\"Iid\")");

            sqlBuilder.Append(" LEFT JOIN (SELECT \"FileRevision\".\"Container\" AS \"Iid\", array_agg(\"FileRevision\".\"Iid\"::text) AS \"FileRevision\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"FileRevision\"", this.GetFileRevisionDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"File\" ON \"FileRevision\".\"Container\" = \"File\".\"Iid\"", this.GetFileDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"FileRevision\".\"Container\") AS \"File_FileRevision\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"File_Audit\".\"Actor\", \"File_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"File_Audit\" AS \"File_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"File_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"File_Audit\".\"Iid\", \"File_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"File\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetFile_CategoryDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"File\",\"Category\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"File_Category\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"File_Category_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
