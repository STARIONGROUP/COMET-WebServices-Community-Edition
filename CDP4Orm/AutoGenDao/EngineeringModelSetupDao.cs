...
namespace CDP4Orm.Dao
{
    ...
    public partial class EngineeringModelSetupDao : DefinedThingDao, IEngineeringModelSetupDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.EngineeringModelSetup"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.EngineeringModelSetup>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as EngineeringModelSetup);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.EngineeringModelSetup engineeringModelSetup, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, engineeringModelSetup, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, engineeringModelSetup, container);
                ...
                foreach (var item in engineeringModelSetup.ActiveDomain)
                {
                    await this.AddActiveDomainAsync(transaction, partition, engineeringModelSetup.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, engineeringModelSetup, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.EngineeringModelSetup engineeringModelSetup, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, engineeringModelSetup, container);
            ...
            foreach (var item in engineeringModelSetup.ActiveDomain)
            {
                await this.UpsertActiveDomainAsync(transaction, partition, engineeringModelSetup.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "ActiveDomain":
                    {
                        isCreated = await this.AddActiveDomainAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddActiveDomainAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid activeDomain)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertActiveDomainAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid activeDomain)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.EngineeringModelSetup engineeringModelSetup, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, engineeringModelSetup, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, engineeringModelSetup, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, engineeringModelSetup, container);
        }
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "ActiveDomain":
                    {
                        isDeleted = await this.DeleteActiveDomainAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// </returns>
        public async Task<bool> DeleteActiveDomainAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid activeDomain)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
