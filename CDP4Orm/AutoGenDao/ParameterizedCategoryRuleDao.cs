// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParameterizedCategoryRuleDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class ParameterizedCategoryRuleDao : RuleDao, IParameterizedCategoryRuleDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.ParameterizedCategoryRule"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.ParameterizedCategoryRule>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<ParameterizedCategoryRule>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"ParameterizedCategoryRule_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as ParameterizedCategoryRule);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ParameterizedCategoryRule parameterizedCategoryRule, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, parameterizedCategoryRule, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, parameterizedCategoryRule, container);
                ...
                foreach (var item in parameterizedCategoryRule.ParameterType)
                {
                    await this.AddParameterTypeAsync(transaction, partition, parameterizedCategoryRule.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, parameterizedCategoryRule, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ParameterizedCategoryRule parameterizedCategoryRule, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, parameterizedCategoryRule, container);
            ...
            foreach (var item in parameterizedCategoryRule.ParameterType)
            {
                await this.UpsertParameterTypeAsync(transaction, partition, parameterizedCategoryRule.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "ParameterType":
                    {
                        isCreated = await this.AddParameterTypeAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddParameterTypeAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid parameterType)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertParameterTypeAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid parameterType)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ParameterizedCategoryRule parameterizedCategoryRule, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, parameterizedCategoryRule, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, parameterizedCategoryRule, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, parameterizedCategoryRule, container);
        }
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "ParameterType":
                    {
                        isDeleted = await this.DeleteParameterTypeAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// </returns>
        public async Task<bool> DeleteParameterTypeAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid parameterType)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="ParameterizedCategoryRuleDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"ParameterizedCategoryRule\".\"Category\",");
            ...
            sqlBuilder.Append(" COALESCE(\"ParameterizedCategoryRule_ParameterType\".\"ParameterType\",'{}'::text[]) AS \"ParameterType\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ParameterizedCategoryRule\" USING (\"Iid\")", this.GetParameterizedCategoryRuleDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"ParameterizedCategoryRule\" AS \"Iid\", array_agg(\"ParameterType\"::text) AS \"ParameterType\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ParameterizedCategoryRule_ParameterType\"", this.GetParameterizedCategoryRule_ParameterTypeDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ParameterizedCategoryRule\" ON \"ParameterizedCategoryRule\" = \"Iid\"", this.GetParameterizedCategoryRuleDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"ParameterizedCategoryRule\") AS \"ParameterizedCategoryRule_ParameterType\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"ParameterizedCategoryRule_Audit\".\"Actor\", \"ParameterizedCategoryRule_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterizedCategoryRule_Audit\" AS \"ParameterizedCategoryRule_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"ParameterizedCategoryRule_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"ParameterizedCategoryRule_Audit\".\"Iid\", \"ParameterizedCategoryRule_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"Rule\".\"ValueTypeDictionary\" || \"ParameterizedCategoryRule\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetParameterizedCategoryRuleDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Category\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterizedCategoryRule\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterizedCategoryRule_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetParameterizedCategoryRule_ParameterTypeDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"ParameterizedCategoryRule\",\"ParameterType\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterizedCategoryRule_ParameterType\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterizedCategoryRule_ParameterType_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
