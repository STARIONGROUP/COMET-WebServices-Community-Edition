// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RatioScaleDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class RatioScaleDao : MeasurementScaleDao, IRatioScaleDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.RatioScale"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.RatioScale>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<RatioScale>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"RatioScale_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as RatioScale);
                            }
                        }
                    }
                }
                else
                {
                    ...
                    sqlBuilder.AppendFormat(" WHERE {0} -> 'ClassKind' = 'RatioScale'", this.GetValueTypeSet());
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RatioScale ratioScale, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, ratioScale, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, ratioScale, container);
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, ratioScale, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RatioScale ratioScale, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, ratioScale, container);
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RatioScale ratioScale, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, ratioScale, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, ratioScale, container);
            }

            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, ratioScale, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="RatioScaleDao" />
        /// </summary>
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"RatioScale_Audit\".\"Actor\", \"RatioScale_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"RatioScale_Audit\" AS \"RatioScale_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"RatioScale_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"RatioScale_Audit\".\"Iid\", \"RatioScale_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"MeasurementScale\".\"ValueTypeDictionary\" || \"RatioScale\".\"ValueTypeDictionary\"";
