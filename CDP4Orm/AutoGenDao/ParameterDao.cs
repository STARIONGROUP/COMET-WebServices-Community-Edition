// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParameterDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class ParameterDao : ParameterOrOverrideBaseDao, IParameterDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Parameter"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Parameter>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"Parameter_Cache\"", partition);
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Parameter parameter, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, parameter, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, parameter, container);
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, parameter, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Parameter parameter, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, parameter, container);
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Parameter parameter, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, parameter, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, parameter, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, parameter, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="ParameterDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"Parameter\".\"Container\",");
            ...
            sqlBuilder.Append(" \"Parameter\".\"RequestedBy\",");
            ...
            sqlBuilder.Append(" COALESCE(\"Parameter_ValueSet\".\"ValueSet\",'{}'::text[]) AS \"ValueSet\",");

            sqlBuilder.Remove(sqlBuilder.Length - 1, 1);
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Thing\"", this.GetThingDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ParameterBase\" USING (\"Iid\")", this.GetParameterBaseDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ParameterOrOverrideBase\" USING (\"Iid\")", this.GetParameterOrOverrideBaseDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Parameter\" USING (\"Iid\")", this.GetParameterDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ParameterSubscription\"", this.GetParameterSubscriptionDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ParameterOrOverrideBase\" ON \"ParameterSubscription\".\"Container\" = \"ParameterOrOverrideBase\".\"Iid\"", this.GetParameterOrOverrideBaseDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"ParameterSubscription\".\"Container\") AS \"ParameterOrOverrideBase_ParameterSubscription\" USING (\"Iid\")");

            sqlBuilder.Append(" LEFT JOIN (SELECT \"ParameterValueSet\".\"Container\" AS \"Iid\", array_agg(\"ParameterValueSet\".\"Iid\"::text) AS \"ValueSet\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ParameterValueSet\"", this.GetParameterValueSetDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Parameter\" ON \"ParameterValueSet\".\"Container\" = \"Parameter\".\"Iid\"", this.GetParameterDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"ParameterValueSet\".\"Container\") AS \"Parameter_ValueSet\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Parameter_Audit\".\"Actor\", \"Parameter_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Parameter_Audit\" AS \"Parameter_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"Parameter_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"Parameter_Audit\".\"Iid\", \"Parameter_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"ParameterBase\".\"ValueTypeDictionary\" || \"ParameterOrOverrideBase\".\"ValueTypeDictionary\" || \"Parameter\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetParameterOrOverrideBaseDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterOrOverrideBase\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterOrOverrideBase_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetParameterSubscriptionDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Container\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterSubscription\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterSubscription_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetParameterValueSetDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Container\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterValueSet\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ParameterValueSet_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
