// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CategoryDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class CategoryDao : DefinedThingDao, ICategoryDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Category"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Category>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Category category, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, category, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, category, container);
                ...
                foreach (var item in category.PermissibleClass)
                {
                    await this.AddPermissibleClassAsync(transaction, partition, category.Iid, item);
                }

                foreach (var item in category.SuperCategory)
                {
                    await this.AddSuperCategoryAsync(transaction, partition, category.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, category, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Category category, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, category, container);
            ...
            foreach (var item in category.PermissibleClass)
            {
                await this.UpsertPermissibleClassAsync(transaction, partition, category.Iid, item);
            }

            foreach (var item in category.SuperCategory)
            {
                await this.UpsertSuperCategoryAsync(transaction, partition, category.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "PermissibleClass":
                    {
                        isCreated = await this.AddPermissibleClassAsync(transaction, partition, iid, (CDP4Common.CommonData.ClassKind)value);
                        break;
                    }

                case "SuperCategory":
                    {
                        isCreated = await this.AddSuperCategoryAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddPermissibleClassAsync(NpgsqlTransaction transaction, string partition, Guid iid, CDP4Common.CommonData.ClassKind permissibleClass)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertPermissibleClassAsync(NpgsqlTransaction transaction, string partition, Guid iid, CDP4Common.CommonData.ClassKind permissibleClass)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddSuperCategoryAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid superCategory)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertSuperCategoryAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid superCategory)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Category category, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, category, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, category, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, category, container);
        }
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "PermissibleClass":
                    {
                        isDeleted = await this.DeletePermissibleClassAsync(transaction, partition, iid, (CDP4Common.CommonData.ClassKind)value);
                        break;
                    }

                case "SuperCategory":
                    {
                        isDeleted = await this.DeleteSuperCategoryAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// </returns>
        public async Task<bool> DeletePermissibleClassAsync(NpgsqlTransaction transaction, string partition, Guid iid, CDP4Common.CommonData.ClassKind permissibleClass)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully removed as result.
        /// </returns>
        public async Task<bool> DeleteSuperCategoryAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid superCategory)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="CategoryDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"Category\".\"Container\",");
            ...
            sqlBuilder.Append(" COALESCE(\"Category_SuperCategory\".\"SuperCategory\",'{}'::text[]) AS \"SuperCategory\",");
            sqlBuilder.Append(" COALESCE(\"Category_PermissibleClass\".\"PermissibleClass\",'{}'::text[]) AS \"PermissibleClass\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Category\" USING (\"Iid\")", this.GetCategoryDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Category\" AS \"Iid\", array_agg(\"SuperCategory\"::text) AS \"SuperCategory\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Category_SuperCategory\"", this.GetCategory_SuperCategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Category\" ON \"Category\" = \"Iid\"", this.GetCategoryDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Category\") AS \"Category_SuperCategory\" USING (\"Iid\")");

            sqlBuilder.Append(" LEFT JOIN (SELECT \"Category\" AS \"Iid\", array_agg(\"PermissibleClass\"::text) AS \"PermissibleClass\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Category_PermissibleClass\"", this.GetCategory_PermissibleClassDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Category\" ON \"Category\"= \"Iid\"", this.GetCategoryDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Category\") AS \"Category_PermissibleClass\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Category_Audit\".\"Actor\", \"Category_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Category_Audit\" AS \"Category_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"Category_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"Category_Audit\".\"Iid\", \"Category_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"Category\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetCategory_PermissibleClassDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"Category\",\"PermissibleClass\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Category_PermissibleClass\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Category_PermissibleClass_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetCategory_SuperCategoryDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"Category\",\"SuperCategory\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Category_SuperCategory\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Category_SuperCategory_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
