// --------------------------------------------------------------------------------------------------------------------
// <copyright file="OptionDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class OptionDao : DefinedThingDao, IOptionDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Option"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Option>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"Option_Cache\"", partition);
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Option option, long sequence, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, option, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, option, container);
                ...
                foreach (var item in option.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, option.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, option, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Option option, long sequence, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, option, container);
            ...
            foreach (var item in option.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, option.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Option option, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, option, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, option, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, option, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="OptionDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"Option\".\"Container\",");

            sqlBuilder.Append(" \"Option\".\"Sequence\",");
            ...
            sqlBuilder.Append(" COALESCE(\"Option_Category\".\"Category\",'{}'::text[]) AS \"Category\",");
            sqlBuilder.Append(" COALESCE(\"Option_NestedElement\".\"NestedElement\",'{}'::text[]) AS \"NestedElement\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Option\" USING (\"Iid\")", this.GetOptionDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Option\" AS \"Iid\", array_agg(\"Category\"::text) AS \"Category\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Option_Category\"", this.GetOption_CategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Option\" ON \"Option\" = \"Iid\"", this.GetOptionDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Option\") AS \"Option_Category\" USING (\"Iid\")");

            sqlBuilder.Append(" LEFT JOIN (SELECT \"NestedElement\".\"Container\" AS \"Iid\", array_agg(\"NestedElement\".\"Iid\"::text) AS \"NestedElement\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"NestedElement\"", this.GetNestedElementDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Option\" ON \"NestedElement\".\"Container\" = \"Option\".\"Iid\"", this.GetOptionDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"NestedElement\".\"Container\") AS \"Option_NestedElement\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Option_Audit\".\"Actor\", \"Option_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Option_Audit\" AS \"Option_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"Option_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"Option_Audit\".\"Iid\", \"Option_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"Option\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetOption_CategoryDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"Option\",\"Category\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Option_Category\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Option_Category_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
