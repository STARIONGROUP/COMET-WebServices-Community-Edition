// --------------------------------------------------------------------------------------------------------------------
// <copyright file="StakeholderValueDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class StakeholderValueDao : DefinedThingDao, IStakeholderValueDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.StakeholderValue"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.StakeholderValue>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<StakeholderValue>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"StakeholderValue_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as StakeholderValue);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.StakeholderValue stakeholderValue, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, stakeholderValue, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, stakeholderValue, container);
                ...
                foreach (var item in stakeholderValue.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, stakeholderValue.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, stakeholderValue, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.StakeholderValue stakeholderValue, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, stakeholderValue, container);
            ...
            foreach (var item in stakeholderValue.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, stakeholderValue.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.StakeholderValue stakeholderValue, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, stakeholderValue, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, stakeholderValue, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, stakeholderValue, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="StakeholderValueDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"StakeholderValue\".\"Container\",");
            ...
            sqlBuilder.Append(" COALESCE(\"StakeholderValue_Category\".\"Category\",'{}'::text[]) AS \"Category\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"StakeholderValue\" USING (\"Iid\")", this.GetStakeholderValueDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"StakeholderValue\" AS \"Iid\", array_agg(\"Category\"::text) AS \"Category\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"StakeholderValue_Category\"", this.GetStakeholderValue_CategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"StakeholderValue\" ON \"StakeholderValue\" = \"Iid\"", this.GetStakeholderValueDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"StakeholderValue\") AS \"StakeholderValue_Category\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"StakeholderValue_Audit\".\"Actor\", \"StakeholderValue_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"StakeholderValue_Audit\" AS \"StakeholderValue_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"StakeholderValue_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"StakeholderValue_Audit\".\"Iid\", \"StakeholderValue_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"StakeholderValue\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetStakeholderValue_CategoryDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"StakeholderValue\",\"Category\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"StakeholderValue_Category\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"StakeholderValue_Category_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
