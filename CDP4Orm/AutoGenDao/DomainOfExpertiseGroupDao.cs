...
namespace CDP4Orm.Dao
{
    ...
    public partial class DomainOfExpertiseGroupDao : DefinedThingDao, IDomainOfExpertiseGroupDao
    {
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"DomainOfExpertiseGroup\" USING (\"Iid\")", this.GetDomainOfExpertiseGroupDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"DomainOfExpertiseGroup_Domain\"", this.GetDomainOfExpertiseGroup_DomainDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"DomainOfExpertiseGroup\" ON \"DomainOfExpertiseGroup\" = \"Iid\"", this.GetDomainOfExpertiseGroupDataSql(partition, instant));
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetDomainOfExpertiseGroup_DomainDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"DomainOfExpertiseGroup\",\"Domain\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"DomainOfExpertiseGroup_Domain\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"DomainOfExpertiseGroup_Domain_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
