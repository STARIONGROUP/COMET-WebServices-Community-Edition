        ...
        public virtual bool Update(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Participant participant, CDP4Common.DTO.Thing container = null)
        {
            ...
            if (!isHandled)
            {
                ...
                using (var command = new NpgsqlCommand())
                {
                    ...
                    sqlBuilder.AppendFormat(" SET (\"Container\", \"Person\", \"Role\", \"SelectedDomain\", \"ValueTypeDictionary\")");
                    sqlBuilder.AppendFormat(" = (:container, :person, :role, :selectedDomain, \"ValueTypeDictionary\" || :valueTypeDictionary)");
