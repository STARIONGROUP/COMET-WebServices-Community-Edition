// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RequestForWaiverDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    /// <summary>
    /// The RequestForWaiver Data Access Object which acts as an ORM layer to the SQL database.
    /// </summary>
    public partial class RequestForWaiverDao : ContractDeviationDao, IRequestForWaiverDao
    {
        /// <summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.RequestForWaiver"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.RequestForWaiver>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<RequestForWaiver>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"RequestForWaiver_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as RequestForWaiver);
                            }
                        }
                    }
                }
        ...
        /// The requestForWaiver DTO that is to be persisted.
        /// </param>
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RequestForWaiver requestForWaiver, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, requestForWaiver, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, requestForWaiver, container);

                await using (var command = new NpgsqlCommand())
                {
                    var sqlBuilder = new System.Text.StringBuilder();

                    sqlBuilder.AppendFormat("INSERT INTO \"{0}\".\"RequestForWaiver\"", partition);
                    sqlBuilder.AppendFormat(" (\"Iid\")");
                    sqlBuilder.AppendFormat(" VALUES (:iid);");

                    command.Parameters.Add("iid", NpgsqlDbType.Uuid).Value = requestForWaiver.Iid;
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, requestForWaiver, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RequestForWaiver requestForWaiver, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, requestForWaiver, container);
        ...
        /// </param>
        /// <param name="requestForWaiver">
        /// The RequestForWaiver DTO that is to be updated.
        /// </param>
        /// <param name="container">
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RequestForWaiver requestForWaiver, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, requestForWaiver, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, requestForWaiver, container);
            }

            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, requestForWaiver, container);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.RequestForWaiver"/> id that is to be deleted.
        /// </param>
        /// <returns>
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="RequestForWaiverDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"RequestForWaiver\" USING (\"Iid\")", this.GetRequestForWaiverDataSql(partition, instant));
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"RequestForWaiver_Audit\".\"Actor\", \"RequestForWaiver_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"RequestForWaiver_Audit\" AS \"RequestForWaiver_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"RequestForWaiver_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"RequestForWaiver_Audit\".\"Iid\", \"RequestForWaiver_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"GenericAnnotation\".\"ValueTypeDictionary\" || \"EngineeringModelDataAnnotation\".\"ValueTypeDictionary\" || \"ModellingAnnotationItem\".\"ValueTypeDictionary\" || \"ContractDeviation\".\"ValueTypeDictionary\" || \"RequestForWaiver\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetRequestForWaiverDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"RequestForWaiver\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"RequestForWaiver_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
