// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ParameterTypeDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public abstract partial class ParameterTypeDao : DefinedThingDao
    {
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ParameterType parameterType, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, parameterType, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, parameterType, container);
                ...
                foreach (var item in parameterType.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, parameterType.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, parameterType, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ParameterType parameterType, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, parameterType, container);
            ...
            foreach (var item in parameterType.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, parameterType.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ParameterType parameterType, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, parameterType, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, parameterType, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, parameterType, container);
        }
