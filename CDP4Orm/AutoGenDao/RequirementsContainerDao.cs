// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RequirementsContainerDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public abstract partial class RequirementsContainerDao : DefinedThingDao
    {
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RequirementsContainer requirementsContainer, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, requirementsContainer, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, requirementsContainer, container);
                ...
                foreach (var item in requirementsContainer.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, requirementsContainer.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, requirementsContainer, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RequirementsContainer requirementsContainer, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, requirementsContainer, container);
            ...
            foreach (var item in requirementsContainer.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, requirementsContainer.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RequirementsContainer requirementsContainer, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, requirementsContainer, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, requirementsContainer, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, requirementsContainer, container);
        }
