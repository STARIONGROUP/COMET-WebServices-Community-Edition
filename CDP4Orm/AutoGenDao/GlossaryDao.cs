// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GlossaryDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class GlossaryDao : DefinedThingDao, IGlossaryDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Glossary"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Glossary>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"Glossary_Cache\"", partition);
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Glossary glossary, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, glossary, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, glossary, container);
                ...
                foreach (var item in glossary.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, glossary.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, glossary, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Glossary glossary, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, glossary, container);
            ...
            foreach (var item in glossary.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, glossary.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Glossary glossary, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, glossary, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, glossary, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, glossary, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="GlossaryDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"Glossary\".\"Container\",");
            ...
            sqlBuilder.Append(" COALESCE(\"Glossary_Category\".\"Category\",'{}'::text[]) AS \"Category\",");
            sqlBuilder.Append(" COALESCE(\"Glossary_Term\".\"Term\",'{}'::text[]) AS \"Term\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Glossary\" USING (\"Iid\")", this.GetGlossaryDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Glossary\" AS \"Iid\", array_agg(\"Category\"::text) AS \"Category\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Glossary_Category\"", this.GetGlossary_CategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Glossary\" ON \"Glossary\" = \"Iid\"", this.GetGlossaryDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Glossary\") AS \"Glossary_Category\" USING (\"Iid\")");

            sqlBuilder.Append(" LEFT JOIN (SELECT \"Term\".\"Container\" AS \"Iid\", array_agg(\"Term\".\"Iid\"::text) AS \"Term\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Term\"", this.GetTermDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Glossary\" ON \"Term\".\"Container\" = \"Glossary\".\"Iid\"", this.GetGlossaryDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Term\".\"Container\") AS \"Glossary_Term\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Glossary_Audit\".\"Actor\", \"Glossary_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Glossary_Audit\" AS \"Glossary_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"Glossary_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"Glossary_Audit\".\"Iid\", \"Glossary_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"Glossary\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetGlossary_CategoryDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"Glossary\",\"Category\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Glossary_Category\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Glossary_Category_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetTermDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Container\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Term\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Term_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
