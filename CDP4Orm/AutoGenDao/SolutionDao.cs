// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SolutionDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    /// <summary>
    /// The Solution Data Access Object which acts as an ORM layer to the SQL database.
    /// </summary>
    [ExcludeFromCodeCoverage]
    public partial class SolutionDao : GenericAnnotationDao, ISolutionDao
    {
        /// <summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Solution"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Solution>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"Solution_Cache\"", partition);
        ...
        /// </param>
        /// <returns>
        /// A deserialized instance of <see cref="CDP4Common.DTO.Solution"/>.
        /// </returns>
        public virtual CDP4Common.DTO.Solution MapToDto(NpgsqlDataReader reader)
        {
            string tempContent;
            ...
            var dto = new CDP4Common.DTO.Solution(iid, revisionNumber);
        ...
        /// </param>
        /// <param name="solution">
        /// The solution DTO that is to be persisted.
        /// </param>
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Solution solution, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, solution, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, solution, container);

                await using (var command = new NpgsqlCommand())
                {
                    var sqlBuilder = new System.Text.StringBuilder();

                    sqlBuilder.AppendFormat("INSERT INTO \"{0}\".\"Solution\"", partition);
                    sqlBuilder.AppendFormat(" (\"Iid\", \"Container\", \"Author\", \"Owner\")");
                    sqlBuilder.AppendFormat(" VALUES (:iid, :container, :author, :owner);");

                    command.Parameters.Add("iid", NpgsqlDbType.Uuid).Value = solution.Iid;
                    command.Parameters.Add("container", NpgsqlDbType.Uuid).Value = container.Iid;
                    command.Parameters.Add("author", NpgsqlDbType.Uuid).Value = !this.IsDerived(solution, "Author") ? solution.Author : Utils.NullableValue(null);
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, solution, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Solution solution, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, solution, container);
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                sqlBuilder.Append(" SET (\"Container\", \"Author\", \"Owner\")");
                sqlBuilder.Append(" = (:container, :author, :owner);");
        ...
        /// </param>
        /// <param name="solution">
        /// The Solution DTO that is to be updated.
        /// </param>
        /// <param name="container">
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Solution solution, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, solution, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, solution, container);

                await using (var command = new NpgsqlCommand())
                {
                    var sqlBuilder = new System.Text.StringBuilder();
                    sqlBuilder.AppendFormat("UPDATE \"{0}\".\"Solution\"", partition);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, solution, container);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.Solution"/> id that is to be deleted.
        /// </param>
        /// <returns>
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="SolutionDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"Solution\".\"Container\",");

            sqlBuilder.Append(" NULL::bigint AS \"Sequence\",");

            sqlBuilder.Append(" \"Actor\",");

            sqlBuilder.Append(" \"Solution\".\"Author\",");

            sqlBuilder.Append(" \"Solution\".\"Owner\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Solution\" USING (\"Iid\")", this.GetSolutionDataSql(partition, instant));
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Solution_Audit\".\"Actor\", \"Solution_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Solution_Audit\" AS \"Solution_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"Solution_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"Solution_Audit\".\"Iid\", \"Solution_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"GenericAnnotation\".\"ValueTypeDictionary\" || \"Solution\".\"ValueTypeDictionary\"";
