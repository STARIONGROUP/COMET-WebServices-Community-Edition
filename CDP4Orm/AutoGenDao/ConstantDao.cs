// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ConstantDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class ConstantDao : DefinedThingDao, IConstantDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Constant"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Constant>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Constant constant, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, constant, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, constant, container);
                ...
                foreach (var item in constant.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, constant.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, constant, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Constant constant, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, constant, container);
            ...
            foreach (var item in constant.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, constant.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Constant constant, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, constant, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, constant, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, constant, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="ConstantDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"Constant\".\"Container\",");

            sqlBuilder.Append(" NULL::bigint AS \"Sequence\",");

            sqlBuilder.Append(" \"Actor\",");

            sqlBuilder.Append(" \"Constant\".\"ParameterType\",");

            sqlBuilder.Append(" \"Constant\".\"Scale\",");
            ...
            sqlBuilder.Append(" COALESCE(\"Constant_Category\".\"Category\",'{}'::text[]) AS \"Category\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Constant\" USING (\"Iid\")", this.GetConstantDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Constant\" AS \"Iid\", array_agg(\"Category\"::text) AS \"Category\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Constant_Category\"", this.GetConstant_CategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Constant\" ON \"Constant\" = \"Iid\"", this.GetConstantDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Constant\") AS \"Constant_Category\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Constant_Audit\".\"Actor\", \"Constant_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Constant_Audit\" AS \"Constant_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"Constant_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"Constant_Audit\".\"Iid\", \"Constant_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"Constant\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetConstant_CategoryDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"Constant\",\"Category\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Constant_Category\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Constant_Category_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
