...
namespace CDP4Orm.Dao
{
    ...
    public partial class ChangeRequestDao : ContractDeviationDao, IChangeRequestDao
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="CDP4Common.DTO.ChangeRequest"/>.
        /// </returns>
        public virtual IEnumerable<CDP4Common.DTO.ChangeRequest> Read(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (thing != null)
                            {
                                yield return thing as ChangeRequest;
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual bool Write(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ChangeRequest changeRequest, CDP4Common.DTO.Thing container = null)
        {
            bool isHandled;
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWrite = this.BeforeWrite(transaction, partition, changeRequest, container, out isHandled, valueTypeDictionaryAdditions);
            if (!isHandled)
            {
                beforeWrite = beforeWrite && base.Write(transaction, partition, changeRequest, container);
        ...
        /// </returns>
        public virtual bool Update(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ChangeRequest changeRequest, CDP4Common.DTO.Thing container = null)
        {
            bool isHandled;
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdate = this.BeforeUpdate(transaction, partition, changeRequest, container, out isHandled, valueTypeDictionaryAdditions);
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && base.Update(transaction, partition, changeRequest, container);
            }

            return this.AfterUpdate(beforeUpdate, transaction, partition, changeRequest, container);
        }
