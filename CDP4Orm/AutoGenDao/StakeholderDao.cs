// --------------------------------------------------------------------------------------------------------------------
// <copyright file="StakeholderDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class StakeholderDao : DefinedThingDao, IStakeholderDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Stakeholder"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Stakeholder>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as Stakeholder);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Stakeholder stakeholder, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, stakeholder, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, stakeholder, container);
                ...
                foreach (var item in stakeholder.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, stakeholder.Iid, item);
                }

                foreach (var item in stakeholder.StakeholderValue)
                {
                    await this.AddStakeholderValueAsync(transaction, partition, stakeholder.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, stakeholder, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Stakeholder stakeholder, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, stakeholder, container);
            ...
            foreach (var item in stakeholder.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, stakeholder.Iid, item);
            }

            foreach (var item in stakeholder.StakeholderValue)
            {
                await this.UpsertStakeholderValueAsync(transaction, partition, stakeholder.Iid, item);
            }
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Stakeholder stakeholder, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, stakeholder, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, stakeholder, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, stakeholder, container);
        }
