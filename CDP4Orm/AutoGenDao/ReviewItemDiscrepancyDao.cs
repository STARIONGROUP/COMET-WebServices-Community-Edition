// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ReviewItemDiscrepancyDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class ReviewItemDiscrepancyDao : ModellingAnnotationItemDao, IReviewItemDiscrepancyDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.ReviewItemDiscrepancy"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.ReviewItemDiscrepancy>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<ReviewItemDiscrepancy>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"ReviewItemDiscrepancy_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as ReviewItemDiscrepancy);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ReviewItemDiscrepancy reviewItemDiscrepancy, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, reviewItemDiscrepancy, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, reviewItemDiscrepancy, container);
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, reviewItemDiscrepancy, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ReviewItemDiscrepancy reviewItemDiscrepancy, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, reviewItemDiscrepancy, container);
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ReviewItemDiscrepancy reviewItemDiscrepancy, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, reviewItemDiscrepancy, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, reviewItemDiscrepancy, container);
            }

            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, reviewItemDiscrepancy, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="ReviewItemDiscrepancyDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" COALESCE(\"ReviewItemDiscrepancy_Solution\".\"Solution\",'{}'::text[]) AS \"Solution\",");
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ReviewItemDiscrepancy\" USING (\"Iid\")", this.GetReviewItemDiscrepancyDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Solution\".\"Container\" AS \"Iid\", array_agg(\"Solution\".\"Iid\"::text) AS \"Solution\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Solution\"", this.GetSolutionDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ReviewItemDiscrepancy\" ON \"Solution\".\"Container\" = \"ReviewItemDiscrepancy\".\"Iid\"", this.GetReviewItemDiscrepancyDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Solution\".\"Container\") AS \"ReviewItemDiscrepancy_Solution\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"ReviewItemDiscrepancy_Audit\".\"Actor\", \"ReviewItemDiscrepancy_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ReviewItemDiscrepancy_Audit\" AS \"ReviewItemDiscrepancy_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"ReviewItemDiscrepancy_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"ReviewItemDiscrepancy_Audit\".\"Iid\", \"ReviewItemDiscrepancy_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"GenericAnnotation\".\"ValueTypeDictionary\" || \"EngineeringModelDataAnnotation\".\"ValueTypeDictionary\" || \"ModellingAnnotationItem\".\"ValueTypeDictionary\" || \"ReviewItemDiscrepancy\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetReviewItemDiscrepancyDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ReviewItemDiscrepancy\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ReviewItemDiscrepancy_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetSolutionDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Container\", \"Author\", \"Owner\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Solution\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Solution_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
