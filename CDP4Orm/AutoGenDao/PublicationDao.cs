...
namespace CDP4Orm.Dao
{
    ...
    public partial class PublicationDao : ThingDao, IPublicationDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.Publication"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.Publication>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as Publication);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Publication publication, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, publication, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, publication, container);
                ...
                foreach (var item in publication.Domain)
                {
                    await this.AddDomainAsync(transaction, partition, publication.Iid, item);
                }

                foreach (var item in publication.PublishedParameter)
                {
                    await this.AddPublishedParameterAsync(transaction, partition, publication.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, publication, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Publication publication, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, publication, container);
            ...
            foreach (var item in publication.Domain)
            {
                await this.UpsertDomainAsync(transaction, partition, publication.Iid, item);
            }

            foreach (var item in publication.PublishedParameter)
            {
                await this.UpsertPublishedParameterAsync(transaction, partition, publication.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                ...
                case "PublishedParameter":
                    {
                        isCreated = await this.AddPublishedParameterAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddPublishedParameterAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid publishedParameter)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertPublishedParameterAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid publishedParameter)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.Publication publication, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, publication, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, publication, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, publication, container);
        }
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                ...
                case "PublishedParameter":
                    {
                        isDeleted = await this.DeletePublishedParameterAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully removed as result.
        /// </returns>
        public async Task<bool> DeletePublishedParameterAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid publishedParameter)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
