...
namespace CDP4Orm.Dao
{
    ...
    public partial class FileTypeDao : DefinedThingDao, IFileTypeDao
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="CDP4Common.DTO.FileType"/>.
        /// </returns>
        public virtual IEnumerable<CDP4Common.DTO.FileType> Read(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (thing != null)
                            {
                                yield return thing as FileType;
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual bool Write(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.FileType fileType, CDP4Common.DTO.Thing container = null)
        {
            bool isHandled;
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWrite = this.BeforeWrite(transaction, partition, fileType, container, out isHandled, valueTypeDictionaryAdditions);
            if (!isHandled)
            {
                beforeWrite = beforeWrite && base.Write(transaction, partition, fileType, container);
                ...
                fileType.Category.ForEach(x => this.AddCategory(transaction, partition, fileType.Iid, x));
            }
        ...
        /// </returns>
        public virtual bool Update(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.FileType fileType, CDP4Common.DTO.Thing container = null)
        {
            bool isHandled;
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdate = this.BeforeUpdate(transaction, partition, fileType, container, out isHandled, valueTypeDictionaryAdditions);
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && base.Update(transaction, partition, fileType, container);
            ...
            return this.AfterUpdate(beforeUpdate, transaction, partition, fileType, container);
        }
