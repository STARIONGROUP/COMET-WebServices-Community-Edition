// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SampledFunctionParameterTypeDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class SampledFunctionParameterTypeDao : ParameterTypeDao, ISampledFunctionParameterTypeDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.SampledFunctionParameterType"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.SampledFunctionParameterType>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<SampledFunctionParameterType>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"SampledFunctionParameterType_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as SampledFunctionParameterType);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.SampledFunctionParameterType sampledFunctionParameterType, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, sampledFunctionParameterType, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, sampledFunctionParameterType, container);
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, sampledFunctionParameterType, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.SampledFunctionParameterType sampledFunctionParameterType, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, sampledFunctionParameterType, container);
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.SampledFunctionParameterType sampledFunctionParameterType, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, sampledFunctionParameterType, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, sampledFunctionParameterType, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, sampledFunctionParameterType, container);
        }
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"SampledFunctionParameterType\" USING (\"Iid\")", this.GetSampledFunctionParameterTypeDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"DependentParameterTypeAssignment\"", this.GetDependentParameterTypeAssignmentDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"SampledFunctionParameterType\" ON \"DependentParameterTypeAssignment\".\"Container\" = \"SampledFunctionParameterType\".\"Iid\"", this.GetSampledFunctionParameterTypeDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"IndependentParameterTypeAssignment\"", this.GetIndependentParameterTypeAssignmentDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"SampledFunctionParameterType\" ON \"IndependentParameterTypeAssignment\".\"Container\" = \"SampledFunctionParameterType\".\"Iid\"", this.GetSampledFunctionParameterTypeDataSql(partition, instant));
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetSampledFunctionParameterTypeDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"SampledFunctionParameterType\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"SampledFunctionParameterType_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
