// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TelephoneNumberDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class TelephoneNumberDao : ThingDao, ITelephoneNumberDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.TelephoneNumber"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.TelephoneNumber>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<TelephoneNumber>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"TelephoneNumber_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as TelephoneNumber);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.TelephoneNumber telephoneNumber, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, telephoneNumber, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, telephoneNumber, container);
                ...
                foreach (var item in telephoneNumber.VcardType)
                {
                    await this.AddVcardTypeAsync(transaction, partition, telephoneNumber.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, telephoneNumber, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.TelephoneNumber telephoneNumber, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, telephoneNumber, container);
            ...
            foreach (var item in telephoneNumber.VcardType)
            {
                await this.UpsertVcardTypeAsync(transaction, partition, telephoneNumber.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "VcardType":
                    {
                        isCreated = await this.AddVcardTypeAsync(transaction, partition, iid, (CDP4Common.SiteDirectoryData.VcardTelephoneNumberKind)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddVcardTypeAsync(NpgsqlTransaction transaction, string partition, Guid iid, CDP4Common.SiteDirectoryData.VcardTelephoneNumberKind vcardType)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertVcardTypeAsync(NpgsqlTransaction transaction, string partition, Guid iid, CDP4Common.SiteDirectoryData.VcardTelephoneNumberKind vcardType)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.TelephoneNumber telephoneNumber, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, telephoneNumber, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, telephoneNumber, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, telephoneNumber, container);
        }
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "VcardType":
                    {
                        isDeleted = await this.DeleteVcardTypeAsync(transaction, partition, iid, (CDP4Common.SiteDirectoryData.VcardTelephoneNumberKind)value);
                        break;
                    }
        ...
        /// </returns>
        public async Task<bool> DeleteVcardTypeAsync(NpgsqlTransaction transaction, string partition, Guid iid, CDP4Common.SiteDirectoryData.VcardTelephoneNumberKind vcardType)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="TelephoneNumberDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"TelephoneNumber\".\"Container\",");
            ...
            sqlBuilder.Append(" COALESCE(\"TelephoneNumber_VcardType\".\"VcardType\",'{}'::text[]) AS \"VcardType\",");

            sqlBuilder.Remove(sqlBuilder.Length - 1, 1);
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Thing\"", this.GetThingDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"TelephoneNumber\" USING (\"Iid\")", this.GetTelephoneNumberDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"TelephoneNumber\" AS \"Iid\", array_agg(\"VcardType\"::text) AS \"VcardType\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"TelephoneNumber_VcardType\"", this.GetTelephoneNumber_VcardTypeDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"TelephoneNumber\" ON \"TelephoneNumber\"= \"Iid\"", this.GetTelephoneNumberDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"TelephoneNumber\") AS \"TelephoneNumber_VcardType\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"TelephoneNumber_Audit\".\"Actor\", \"TelephoneNumber_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"TelephoneNumber_Audit\" AS \"TelephoneNumber_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"TelephoneNumber_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"TelephoneNumber_Audit\".\"Iid\", \"TelephoneNumber_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"TelephoneNumber\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetTelephoneNumber_VcardTypeDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"TelephoneNumber\",\"VcardType\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"TelephoneNumber_VcardType\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"TelephoneNumber_VcardType_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
