// --------------------------------------------------------------------------------------------------------------------
// <copyright file="OrdinalScaleDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class OrdinalScaleDao : MeasurementScaleDao, IOrdinalScaleDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.OrdinalScale"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.OrdinalScale>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<OrdinalScale>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"OrdinalScale_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as OrdinalScale);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.OrdinalScale ordinalScale, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, ordinalScale, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, ordinalScale, container);
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, ordinalScale, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.OrdinalScale ordinalScale, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, ordinalScale, container);
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.OrdinalScale ordinalScale, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, ordinalScale, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, ordinalScale, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, ordinalScale, container);
        }
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="OrdinalScaleDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"OrdinalScale\" USING (\"Iid\")", this.GetOrdinalScaleDataSql(partition, instant));
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"OrdinalScale_Audit\".\"Actor\", \"OrdinalScale_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"OrdinalScale_Audit\" AS \"OrdinalScale_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"OrdinalScale_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"OrdinalScale_Audit\".\"Iid\", \"OrdinalScale_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"DefinedThing\".\"ValueTypeDictionary\" || \"MeasurementScale\".\"ValueTypeDictionary\" || \"OrdinalScale\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetOrdinalScaleDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"OrdinalScale\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"OrdinalScale_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
