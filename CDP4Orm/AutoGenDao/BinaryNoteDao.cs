// --------------------------------------------------------------------------------------------------------------------
// <copyright file="BinaryNoteDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    /// <summary>
    /// The BinaryNote Data Access Object which acts as an ORM layer to the SQL database.
    /// </summary>
    [ExcludeFromCodeCoverage]
    public partial class BinaryNoteDao : NoteDao, IBinaryNoteDao
    {
        /// <summary>
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.BinaryNote"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.BinaryNote>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<BinaryNote>();
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as BinaryNote);
                            }
                        }
                    }
                }
        ...
        /// </param>
        /// <returns>
        /// A deserialized instance of <see cref="CDP4Common.DTO.BinaryNote"/>.
        /// </returns>
        public virtual CDP4Common.DTO.BinaryNote MapToDto(NpgsqlDataReader reader)
        {
            string tempCaption;
            ...
            var dto = new CDP4Common.DTO.BinaryNote(iid, revisionNumber);
            ...
            dto.FileType = Guid.Parse(reader["FileType"].ToString());
            dto.Owner = Guid.Parse(reader["Owner"].ToString());

            if (valueDict.TryGetValue("Caption", out tempCaption))
            {
                dto.Caption = tempCaption.UnEscape();
            }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.BinaryNote binaryNote, long sequence, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, binaryNote, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, binaryNote, sequence, container);
            ...
            return await this.AfterWriteAsync(beforeWrite, transaction, partition, binaryNote, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.BinaryNote binaryNote, long sequence, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, binaryNote, sequence, container);
        ...
        /// </param>
        /// <param name="binaryNote">
        /// The BinaryNote DTO that is to be updated.
        /// </param>
        /// <param name="container">
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.BinaryNote binaryNote, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, binaryNote, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, binaryNote, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, binaryNote, container);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.BinaryNote"/> id that is to be deleted.
        /// </param>
        /// <returns>
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="BinaryNoteDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"BinaryNote\".\"FileType\",");
            sqlBuilder.Append(" COALESCE(\"Thing_ExcludedDomain\".\"ExcludedDomain\",'{}'::text[]) AS \"ExcludedDomain\",");
            sqlBuilder.Append(" COALESCE(\"Thing_ExcludedPerson\".\"ExcludedPerson\",'{}'::text[]) AS \"ExcludedPerson\",");
            sqlBuilder.Append(" COALESCE(\"Note_Category\".\"Category\",'{}'::text[]) AS \"Category\",");

            sqlBuilder.Remove(sqlBuilder.Length - 1, 1);
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Thing\"", this.GetThingDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Note\" USING (\"Iid\")", this.GetNoteDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"BinaryNote\" USING (\"Iid\")", this.GetBinaryNoteDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"Note\" AS \"Iid\", array_agg(\"Category\"::text) AS \"Category\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Note_Category\"", this.GetNote_CategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"Note\" ON \"Note\" = \"Iid\"", this.GetNoteDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"Note\") AS \"Note_Category\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"BinaryNote_Audit\".\"Actor\", \"BinaryNote_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"BinaryNote_Audit\" AS \"BinaryNote_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"BinaryNote_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"BinaryNote_Audit\".\"Iid\", \"BinaryNote_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"Note\".\"ValueTypeDictionary\" || \"BinaryNote\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetBinaryNoteDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"FileType\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"BinaryNote\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"BinaryNote_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
