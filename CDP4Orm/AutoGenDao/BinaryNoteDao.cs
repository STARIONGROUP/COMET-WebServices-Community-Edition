...
namespace CDP4Orm.Dao
{
    ...
    public partial class BinaryNoteDao : NoteDao, IBinaryNoteDao
    {
        ...
        /// </param>
        /// <returns>
        /// List of instances of <see cref="CDP4Common.DTO.BinaryNote"/>.
        /// </returns>
        public virtual IEnumerable<CDP4Common.DTO.BinaryNote> Read(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
                ...
                if (isCachedDtoReadEnabledAndInstant)
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (thing != null)
                            {
                                yield return thing as BinaryNote;
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual bool Write(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.BinaryNote binaryNote, long sequence, CDP4Common.DTO.Thing container = null)
        {
            bool isHandled;
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWrite = this.BeforeWrite(transaction, partition, binaryNote, container, out isHandled, valueTypeDictionaryAdditions);
            if (!isHandled)
            {
                beforeWrite = beforeWrite && base.Write(transaction, partition, binaryNote, sequence, container);
        ...
        /// </returns>
        public virtual bool Update(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.BinaryNote binaryNote, CDP4Common.DTO.Thing container = null)
        {
            bool isHandled;
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdate = this.BeforeUpdate(transaction, partition, binaryNote, container, out isHandled, valueTypeDictionaryAdditions);
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && base.Update(transaction, partition, binaryNote, container);
            ...
            return this.AfterUpdate(beforeUpdate, transaction, partition, binaryNote, container);
        }
