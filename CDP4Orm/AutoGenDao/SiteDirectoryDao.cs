        ...
        public virtual bool Update(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.SiteDirectory siteDirectory, CDP4Common.DTO.Thing container = null)
        {
            ...
            if (!isHandled)
            {
                ...
                using (var command = new NpgsqlCommand())
                {
                    ...
                    sqlBuilder.AppendFormat(" SET (\"DefaultParticipantRole\", \"DefaultPersonRole\", \"ValueTypeDictionary\")");
                    sqlBuilder.AppendFormat(" = (:defaultParticipantRole, :defaultPersonRole, \"ValueTypeDictionary\" || :valueTypeDictionary)");
