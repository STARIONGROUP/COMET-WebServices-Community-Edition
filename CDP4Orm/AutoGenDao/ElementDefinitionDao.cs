// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ElementDefinitionDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    /// <summary>
    /// The ElementDefinition Data Access Object which acts as an ORM layer to the SQL database.
    /// </summary>
    public partial class ElementDefinitionDao : ElementBaseDao, IElementDefinitionDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.ElementDefinition"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.ElementDefinition>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"ElementDefinition_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as ElementDefinition);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ElementDefinition elementDefinition, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, elementDefinition, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, elementDefinition, container);
                ...
                foreach (var item in elementDefinition.OrganizationalParticipant)
                {
                    await this.AddOrganizationalParticipantAsync(transaction, partition, elementDefinition.Iid, item);
                }

                foreach (var item in elementDefinition.ReferencedElement)
                {
                    await this.AddReferencedElementAsync(transaction, partition, elementDefinition.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, elementDefinition, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ElementDefinition elementDefinition, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, elementDefinition, container);
            ...
            foreach (var item in elementDefinition.OrganizationalParticipant)
            {
                await this.UpsertOrganizationalParticipantAsync(transaction, partition, elementDefinition.Iid, item);
            }

            foreach (var item in elementDefinition.ReferencedElement)
            {
                await this.UpsertReferencedElementAsync(transaction, partition, elementDefinition.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "OrganizationalParticipant":
                    {
                        isCreated = await this.AddOrganizationalParticipantAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }

                case "ReferencedElement":
                    {
                        isCreated = await this.AddReferencedElementAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddOrganizationalParticipantAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid organizationalParticipant)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertOrganizationalParticipantAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid organizationalParticipant)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddReferencedElementAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid referencedElement)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertReferencedElementAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid referencedElement)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ElementDefinition elementDefinition, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, elementDefinition, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, elementDefinition, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, elementDefinition, container);
        }
        ...
        /// </param>
        /// <param name="iid">
        /// The <see cref="CDP4Common.DTO.ElementDefinition"/> id that is the source of each link table record.
        /// </param> 
        /// <param name="value">
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "OrganizationalParticipant":
                    {
                        isDeleted = await this.DeleteOrganizationalParticipantAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }

                case "ReferencedElement":
                    {
                        isDeleted = await this.DeleteReferencedElementAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// </returns>
        public async Task<bool> DeleteOrganizationalParticipantAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid organizationalParticipant)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully removed as result.
        /// </returns>
        public async Task<bool> DeleteReferencedElementAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid referencedElement)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
                sqlBuilder.AppendFormat("DELETE FROM \"{0}\".\"ElementDefinition_ReferencedElement\"", partition);
                sqlBuilder.Append(" WHERE \"ElementDefinition\" = :elementDefinition");
                sqlBuilder.Append(" AND \"ReferencedElement\" = :referencedElement;");
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementBase\" USING (\"Iid\")", this.GetElementBaseDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementDefinition\" USING (\"Iid\")", this.GetElementDefinitionDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ElementBase_Category\"", this.GetElementBase_CategoryDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementBase\" ON \"ElementBase\" = \"Iid\"", this.GetElementBaseDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ElementUsage\"", this.GetElementUsageDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementDefinition\" ON \"ElementUsage\".\"Container\" = \"ElementDefinition\".\"Iid\"", this.GetElementDefinitionDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ElementDefinition_OrganizationalParticipant\"", this.GetElementDefinition_OrganizationalParticipantDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementDefinition\" ON \"ElementDefinition\" = \"Iid\"", this.GetElementDefinitionDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Parameter\"", this.GetParameterDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementDefinition\" ON \"Parameter\".\"Container\" = \"ElementDefinition\".\"Iid\"", this.GetElementDefinitionDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ParameterGroup\"", this.GetParameterGroupDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementDefinition\" ON \"ParameterGroup\".\"Container\" = \"ElementDefinition\".\"Iid\"", this.GetElementDefinitionDataSql(partition, instant));
            ...
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"ElementDefinition_ReferencedElement\"", this.GetElementDefinition_ReferencedElementDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"ElementDefinition\" ON \"ElementDefinition\" = \"Iid\"", this.GetElementDefinitionDataSql(partition, instant));
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetElementUsageDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Container\", \"ElementDefinition\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ElementUsage\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ElementUsage_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetElementDefinition_OrganizationalParticipantDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"ElementDefinition\",\"OrganizationalParticipant\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ElementDefinition_OrganizationalParticipant\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ElementDefinition_OrganizationalParticipant_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetParameterDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Container\", \"RequestedBy\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"Parameter\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"Parameter_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetElementDefinition_ReferencedElementDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"ElementDefinition\",\"ReferencedElement\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"ElementDefinition_ReferencedElement\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"ElementDefinition_ReferencedElement_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
