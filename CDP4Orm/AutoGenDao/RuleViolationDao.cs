// --------------------------------------------------------------------------------------------------------------------
// <copyright file="RuleViolationDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class RuleViolationDao : ThingDao, IRuleViolationDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.RuleViolation"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.RuleViolation>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"RuleViolation_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as RuleViolation);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RuleViolation ruleViolation, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, ruleViolation, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, ruleViolation, container);
                ...
                foreach (var item in ruleViolation.ViolatingThing)
                {
                    await this.AddViolatingThingAsync(transaction, partition, ruleViolation.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, ruleViolation, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RuleViolation ruleViolation, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, ruleViolation, container);
            ...
            foreach (var item in ruleViolation.ViolatingThing)
            {
                await this.UpsertViolatingThingAsync(transaction, partition, ruleViolation.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "ViolatingThing":
                    {
                        isCreated = await this.AddViolatingThingAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddViolatingThingAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid violatingThing)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertViolatingThingAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid violatingThing)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.RuleViolation ruleViolation, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, ruleViolation, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, ruleViolation, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, ruleViolation, container);
        }
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "ViolatingThing":
                    {
                        isDeleted = await this.DeleteViolatingThingAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// </returns>
        public async Task<bool> DeleteViolatingThingAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid violatingThing)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// <summary>
        /// Build a SQL read query for the current <see cref="RuleViolationDao" />
        /// </summary>
        ...
        public override string BuildReadQuery(string partition, DateTime? instant)
        {
            ...
            sqlBuilder.Append(" \"RuleViolation\".\"Container\",");
            ...
            sqlBuilder.Append(" COALESCE(\"RuleViolation_ViolatingThing\".\"ViolatingThing\",'{}'::text[]) AS \"ViolatingThing\",");

            sqlBuilder.Remove(sqlBuilder.Length - 1, 1);
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"Thing\"", this.GetThingDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"RuleViolation\" USING (\"Iid\")", this.GetRuleViolationDataSql(partition, instant));
            ...
            sqlBuilder.Append(" LEFT JOIN (SELECT \"RuleViolation\" AS \"Iid\", array_agg(\"ViolatingThing\"::text) AS \"ViolatingThing\"");
            sqlBuilder.AppendFormat(" FROM ({0}) AS \"RuleViolation_ViolatingThing\"", this.GetRuleViolation_ViolatingThingDataSql(partition, instant));
            sqlBuilder.AppendFormat(" JOIN ({0}) AS \"RuleViolation\" ON \"RuleViolation\"= \"Iid\"", this.GetRuleViolationDataSql(partition, instant));
            sqlBuilder.Append(" GROUP BY \"RuleViolation\") AS \"RuleViolation_ViolatingThing\" USING (\"Iid\")");
        ...
        {
            var sqlBuilder = new StringBuilder();
            sqlBuilder.Append(" LEFT JOIN (SELECT \"RuleViolation_Audit\".\"Actor\", \"RuleViolation_Audit\".\"Iid\"");
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"RuleViolation_Audit\" AS \"RuleViolation_Audit\"", partition);
            sqlBuilder.Append(" WHERE \"RuleViolation_Audit\".\"ValidTo\" = 'infinity'");
            sqlBuilder.Append(" GROUP BY \"RuleViolation_Audit\".\"Iid\", \"RuleViolation_Audit\".\"Actor\") AS \"Actor\" USING (\"Iid\")");
            return sqlBuilder.ToString();
        }
        ...
        public override string GetValueTypeSet() => "\"Thing\".\"ValueTypeDictionary\" || \"RuleViolation\".\"ValueTypeDictionary\"";
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetRuleViolationDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = " \"Iid\", \"ValueTypeDictionary\", \"Container\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"RuleViolation\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"RuleViolation_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
        ...
        /// </param>
        /// <returns>The DataSql string</returns>
        private string GetRuleViolation_ViolatingThingDataSql(string partition, DateTime? instant)
        {
            var sqlBuilder = new StringBuilder();

            var fields = "\"RuleViolation\",\"ViolatingThing\",\"ValidFrom\",\"ValidTo\"";
            sqlBuilder.AppendFormat(" SELECT {0}", fields);
            sqlBuilder.AppendFormat(" FROM \"{0}\".\"RuleViolation_ViolatingThing\"", partition);
            ...
            if (instant.HasValue && instant.Value != DateTime.MaxValue)
            {
                ...
                sqlBuilder.Append(" UNION ALL");
                sqlBuilder.AppendFormat(" SELECT {0}", fields);
                sqlBuilder.AppendFormat(" FROM \"{0}\".\"RuleViolation_ViolatingThing_Audit\"", partition);
                sqlBuilder.Append(" WHERE \"Action\" <> 'I'");
                sqlBuilder.Append(" AND \"ValidFrom\" < :instant");
                sqlBuilder.Append(" AND \"ValidTo\" >= :instant");
            }
