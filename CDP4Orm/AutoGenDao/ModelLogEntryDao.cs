// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ModelLogEntryDao.cs" company="Starion Group S.A.">
...
namespace CDP4Orm.Dao
{
    ...
    public partial class ModelLogEntryDao : ThingDao, IModelLogEntryDao
    {
        ...
        /// </param>
        /// <returns>
        /// An awaitable <see cref="Task"/> having a list of instances of <see cref="CDP4Common.DTO.ModelLogEntry"/> as result.
        /// </returns>
        public virtual async Task<IEnumerable<CDP4Common.DTO.ModelLogEntry>> ReadAsync(NpgsqlTransaction transaction, string partition, IEnumerable<Guid> ids = null, bool isCachedDtoReadEnabledAndInstant = false, DateTime? instant = null)
        {
            var result = new List<ModelLogEntry>();
            ...
            await using (var command = new NpgsqlCommand())
            {
                ...
                {
                    sqlBuilder.AppendFormat("SELECT \"Jsonb\", \"Actor\" FROM \"{0}\".\"ModelLogEntry_Cache\"", partition);
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (thing != null)
                            {
                                result.Add(thing as ModelLogEntry);
                            }
                        }
                    }
                }
        ...
        /// </returns>
        public virtual async Task<bool> WriteAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ModelLogEntry modelLogEntry, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeWriteResult = await this.BeforeWriteAsync(transaction, partition, modelLogEntry, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeWrite = beforeWrite && await base.WriteAsync(transaction, partition, modelLogEntry, container);
                ...
                foreach (var item in modelLogEntry.AffectedDomainIid)
                {
                    await this.AddAffectedDomainIidAsync(transaction, partition, modelLogEntry.Iid, item);
                }

                foreach (var item in modelLogEntry.AffectedItemIid)
                {
                    await this.AddAffectedItemIidAsync(transaction, partition, modelLogEntry.Iid, item);
                }

                foreach (var item in modelLogEntry.Category)
                {
                    await this.AddCategoryAsync(transaction, partition, modelLogEntry.Iid, item);
                }
            }

            return await this.AfterWriteAsync(beforeWrite, transaction, partition, modelLogEntry, container);
        }
        ...
        /// An awaitable <see cref="Task"/> having True if the concept was successfully persisted as result.
        /// </returns>
        public virtual async Task<bool> UpsertAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ModelLogEntry modelLogEntry, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            await base.UpsertAsync(transaction, partition, modelLogEntry, container);
            ...
            foreach (var item in modelLogEntry.AffectedDomainIid)
            {
                await this.UpsertAffectedDomainIidAsync(transaction, partition, modelLogEntry.Iid, item);
            }

            foreach (var item in modelLogEntry.AffectedItemIid)
            {
                await this.UpsertAffectedItemIidAsync(transaction, partition, modelLogEntry.Iid, item);
            }

            foreach (var item in modelLogEntry.Category)
            {
                await this.UpsertCategoryAsync(transaction, partition, modelLogEntry.Iid, item);
            }
        ...
        public override async Task<bool> AddToCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "AffectedDomainIid":
                    {
                        isCreated = await this.AddAffectedDomainIidAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }

                case "AffectedItemIid":
                    {
                        isCreated = await this.AddAffectedItemIidAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddAffectedDomainIidAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid affectedDomainIid)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
                sqlBuilder.AppendFormat("INSERT INTO \"{0}\".\"ModelLogEntry_AffectedDomainIid\"", partition);
                sqlBuilder.AppendFormat(" (\"ModelLogEntry\", \"AffectedDomainIid\")");
                sqlBuilder.Append(" VALUES (:modelLogEntry, :affectedDomainIid);");
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertAffectedDomainIidAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid affectedDomainIid)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> AddAffectedItemIidAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid affectedItemIid)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully created as result.
        /// </returns>
        public async Task<bool> UpsertAffectedItemIidAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid affectedItemIid)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
        ...
        /// </returns>
        public virtual async Task<bool> UpdateAsync(NpgsqlTransaction transaction, string partition, CDP4Common.DTO.ModelLogEntry modelLogEntry, CDP4Common.DTO.Thing container = null)
        {
            var valueTypeDictionaryAdditions = new Dictionary<string, string>();
            var beforeUpdateResult = await this.BeforeUpdateAsync(transaction, partition, modelLogEntry, container, valueTypeDictionaryAdditions);
            ...
            if (!isHandled)
            {
                beforeUpdate = beforeUpdate && await base.UpdateAsync(transaction, partition, modelLogEntry, container);
            ...
            return await this.AfterUpdateAsync(beforeUpdate, transaction, partition, modelLogEntry, container);
        }
        ...
        public override async Task<bool> DeleteFromCollectionPropertyAsync(NpgsqlTransaction transaction, string partition, string propertyName, Guid iid, object value)
        {
            ...
            switch (propertyName)
            {
                case "AffectedDomainIid":
                    {
                        isDeleted = await this.DeleteAffectedDomainIidAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }

                case "AffectedItemIid":
                    {
                        isDeleted = await this.DeleteAffectedItemIidAsync(transaction, partition, iid, (Guid)value);
                        break;
                    }
        ...
        /// </returns>
        public async Task<bool> DeleteAffectedDomainIidAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid affectedDomainIid)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
                sqlBuilder.AppendFormat("DELETE FROM \"{0}\".\"ModelLogEntry_AffectedDomainIid\"", partition);
        ...
        /// An awaitable <see cref="Task"/> having True if the value link was successfully removed as result.
        /// </returns>
        public async Task<bool> DeleteAffectedItemIidAsync(NpgsqlTransaction transaction, string partition, Guid iid, Guid affectedItemIid)
        {
            await using (var command = new NpgsqlCommand())
            {
                var sqlBuilder = new System.Text.StringBuilder();
